...
class TradingAccountRequest(BaseModel):
    ...
    opening_stock: float = 0; purchases: float = 0; carriage_inward: float = 0
    closing_stock: float = 0; sales: float = 0; sales_returns: float = 0
    direct_wages: float = 0; factory_overhead: float = 0

class TradingAccount(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; period: str
    net_sales: float; cost_of_goods_available: float; cost_of_goods_sold: float
    gross_profit: float; gross_profit_pct: float
    opening_stock: float; purchases: float; direct_wages: float; factory_overhead: float
    closing_stock: float
...
async def generate_trading_account(req: TradingAccountRequest):
    ...
    return TradingAccount(
        ...
        net_sales=round(net_sales, 2), cost_of_goods_available=round(cost_of_goods_available, 2),
        cost_of_goods_sold=round(cogs, 2), gross_profit=round(gross_profit, 2),
        gross_profit_pct=round(gross_profit_pct, 2),
        opening_stock=req.opening_stock, purchases=total_purchases,
        direct_wages=req.direct_wages, factory_overhead=req.factory_overhead,
