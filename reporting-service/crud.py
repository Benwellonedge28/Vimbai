...
async def update_dashboard(
    session: AsyncSession, dashboard_id: str, dashboard_data: DashboardUpdate
...
async def create_report_template(
    session: AsyncSession, user_id: str, template_data: ReportTemplateCreate
) -> ReportTemplateInDB:
    ...
    async for record in result:
        ...
        for key, value in record.items():
            if hasattr(value, "iso_format"):
                ...
                rec[key] = [v.iso_format() if hasattr(v, "iso_format") else v for v in value]
            else:
                rec[key] = value
        records.append(rec)
    return records
...
async def create_scheduled_report(
    session: AsyncSession, user_id: str, report_data: ScheduledReportCreate
) -> ScheduledReportInDB:
...
def _dashboard_from_neo4j(node: Dict[str, Any]) -> DashboardInDB:
    return DashboardInDB(
        ...
        widgets=(
            json.loads(node.get("widgets", "[]")) if isinstance(node.get("widgets"), str) else node.get("widgets", [])
        ),
        is_default=node.get("is_default", False),
        user_id=node.get("user_id"),
        created_at=(
            datetime.fromisoformat(node["created_at"].iso_format())
            if hasattr(node["created_at"], "iso_format")
            else datetime.fromisoformat(node["created_at"])
        ),
        updated_at=(
            datetime.fromisoformat(node["updated_at"].iso_format())
            if hasattr(node["updated_at"], "iso_format")
            else datetime.fromisoformat(node["updated_at"])
        ),
    )
...
def _scheduled_report_from_neo4j(node: Dict[str, Any]) -> ScheduledReportInDB:
    return ScheduledReportInDB(
        ...
        recipients=(
            json.loads(node.get("recipients", "[]"))
            if isinstance(node.get("recipients"), str)
            else node.get("recipients", [])
        ),
