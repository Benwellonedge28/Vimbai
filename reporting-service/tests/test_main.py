...
class TestReportGeneration:
    def test_generate_report_no_auth(self):
        response = client.post("/generate", json={"report_type": "balance_sheet", "period": "2026-Q1"})
    ...
    def test_generate_invalid_report_type(self, auth_headers):
        response = client.post(
            "/generate", json={"report_type": "nonexistent_report", "period": "2026-Q1"}, headers=auth_headers
        )
...
class TestReportScheduling:
    def test_schedule_report_no_auth(self):
        response = client.post(
            "/schedule", json={"report_type": "income_statement", "frequency": "monthly", "email": "user@vimbai.com"}
        )
        assert response.status_code in [401, 403, 422]
    ...
    def test_schedule_report_invalid_frequency(self, auth_headers):
        response = client.post(
            "/schedule",
            json={"report_type": "income_statement", "frequency": "invalid", "email": "user@vimbai.com"},
            headers=auth_headers,
        )
        assert response.status_code in [422, 400, 200, 201]
