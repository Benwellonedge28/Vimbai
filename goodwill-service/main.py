...
class CGU(BaseModel):
    ...
    name: str; carrying_value: float
    goodwill_allocated: float = 0
    fair_value: float = 0; value_in_use: float = 0

class ImpairmentTestRequest(BaseModel):
    company_id: str; fiscal_year: int
    cgu: CGU; test_method: str = "higher_of"  # higher_of, fair_value_only, viu_only

class ImpairmentTestResult(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; fiscal_year: int; cgu_name: str
    carrying_value: float; recoverable_amount: float
    impairment_loss: float; goodwill_impaired: float
    other_assets_impaired: float
    test_method: str; is_impaired: bool
