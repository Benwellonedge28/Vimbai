...
class MultimodalProcessor:
    ...
    async def _get_account_details(self, jwt_token: str, account_number: str) -> Optional[Dict]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.accounting_service_url}/accounts/{account_number}", headers=headers)
                response.raise_for_status()
                return response.json()
        except httpx.HTTPStatusError as e:
            print(
                f"Account {account_number} not found in Accounting Service: {e.response.status_code} - {e.response.text}"
            )
            return None
    ...
    async def _map_extracted_data_to_journal_entry(
        self, extracted_data: List[ExtractedField], source_context: Optional[str] = None, jwt_token: str = None
    ) -> Optional[JournalEntryCreate]:
        """
        ...
        if jwt_token:
            ...
            if not debit_acc_details or not credit_acc_details:
                print(
                    f"Inferred account numbers not found in Accounting Service. Debit: {debit_account_number}, Credit: {credit_account_number}. Using pending status."
                )
    ...
    async def send_journal_entry_to_accounting_service(
        self, jwt_token: str, journal_entry: JournalEntryCreate
    ) -> AutomatedJournalEntryResponse:
    ...
    async def _send_transaction_to_fraud_detection_service(
        self, jwt_token: str, transaction_data: TransactionForFraudCheck
    ) -> FraudDetectionResult:
    ...
    async def process_document_ocr(
        self, image_data: Union[str, bytes], source_context: Optional[str] = None, jwt_token: str = None
    ) -> DocumentParseResult:
        """
        ...
        proposed_journal_entry = await self._map_extracted_data_to_journal_entry(
            extracted_data, source_context, jwt_token
        )
        automated_je_response = None
        if proposed_journal_entry:
            # Send to accounting service
            automated_je_response = await self.send_journal_entry_to_accounting_service(
                jwt_token, proposed_journal_entry
            )
    ...
    async def process_audio_asr(
        self, audio_data: Union[str, bytes], source_context: Optional[str] = None
    ) -> AudioParseResult:
        start_time = datetime.now()
        transcribed_text = ""
...
async def enqueue_task(
    task_id: str, processor: MultimodalProcessor, input_data: MultimodalInput, jwt_token: str = None
):
    ...
    try:
        if input_data.document_data:
            result = await processor.process_document_ocr(
                input_data.document_data, input_data.source_context, jwt_token
            )
