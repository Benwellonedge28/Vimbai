...
from multimodal_pipeline_service.database import init_db_schema, Neo4jConnector
...
@app.post("/tasks/", response_model=models.MultimodalProcessingTaskInDB, status_code=status.HTTP_201_CREATED,
...
@app.get("/tasks/", response_model=List[models.MultimodalProcessingTaskInDB],
...
@app.get("/tasks/{task_id}", response_model=models.MultimodalProcessingTaskInDB,
             dependencies=[Depends(check_permission("multimodal.read.tasks"))])
async def get_multimodal_task_by_id(
    task_id: str,
    db_session: AsyncSession = Depends(get_db_session)
):
...
@app.put("/tasks/{task_id}", response_model=models.MultimodalProcessingTaskInDB,
             dependencies=[Depends(check_permission("multimodal.write.tasks"))])
async def update_multimodal_task(
    task_id: str,
    task_update: models.MultimodalProcessingTaskUpdate,
    db_session: AsyncSession = Depends(get_db_session)
):
...
async def delete_multimodal_task(
    task_id: str,
    db_session: AsyncSession = Depends(get_db_session)
):
...
@app.post("/tasks/{task_id}/corrections", response_model=models.UserCorrectionInDB, status_code=status.HTTP_201_CREATED,
...
async def submit_user_correction(
    ...
    await crud.update_multimodal_processing_task(db_session, task_id, models.MultimodalProcessingTaskUpdate(
    ...
    print(f"User correction received for task {task_id}: {correction_data.field_name} from {correction_data.original_value} to {correction_data.corrected_value}")

    return correction

@app.get("/tasks/{task_id}/corrections", response_model=List[models.UserCorrectionInDB],
