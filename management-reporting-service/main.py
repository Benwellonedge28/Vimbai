...
async def generate_management_report(request: ManagementReportingRequest):
    ...
    performance_summary = {
        ...
        "gross_margin": round((total_revenue - request.actuals.get("cogs", 0)) / total_revenue * 100, 2) if total_revenue else 0,
    ...
    kpis = {
        "revenue_variance": round((total_revenue - request.budget.get("revenue", 1)) / request.budget.get("revenue", 1) * 100, 2),
        "cost_variance": round((total_costs - request.budget.get("cogs", 1) - request.budget.get("opex", 1)) / (request.budget.get("cogs", 1) + request.budget.get("opex", 1)) * 100, 2),
