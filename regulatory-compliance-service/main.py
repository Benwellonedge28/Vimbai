...
class RegStatus(str, Enum):
    COMPLIANT = "compliant"; NON_COMPLIANT = "non_compliant"; PENDING_REVIEW = "pending_review"; NOT_APPLICABLE = "n/a"

class Regulation(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; regulation_name: str; jurisdiction: str
    framework: str  # iFRS, IAS, SOX, Basel III, AML, GDPR, PCI_DSS
    requirement: str; status: RegStatus = RegStatus.PENDING_REVIEW
    last_reviewed: str = ""; next_review_due: str = ""
    risk_if_non_compliant: str = "medium"  # low, medium, high, critical

class ComplianceDashboard(BaseModel):
    company_id: str; total_regulations: int
    compliant: int; non_compliant: int; pending: int
    compliance_rate: float; by_framework: Dict[str, Dict] = {}
...
async def update_reg_status(company_id: str, reg_id: str, status: str):
    ...
    from fastapi import HTTPException; raise HTTPException(status_code=404, detail="Regulation not found")
...
async def get_dashboard(company_id: str):
    ...
    critical = [{"regulation": r.regulation_name, "jurisdiction": r.jurisdiction,
                 "framework": r.framework, "status": r.status.value,
                 "risk": r.risk_if_non_compliant}
                for r in items if r.risk_if_non_compliant in ("high", "critical") and r.status != RegStatus.COMPLIANT]
    
    return ComplianceDashboard(
        company_id=company_id, total_regulations=total,
        compliant=compliant, non_compliant=non_compliant, pending=pending,
        compliance_rate=round(rate, 1), by_framework=by_framework,
        by_jurisdiction=by_jur, critical_items=critical
    )
