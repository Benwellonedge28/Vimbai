...
class PurchaseRecord(BaseModel):
    date: str; quantity: int; unit_cost: float

class SaleRecord(BaseModel):
    date: str; quantity: int; unit_price: float

class InventoryValuationRequest(BaseModel):
    company_id: str; method: str  # fifo, lifo, weighted_average, specific_identification
    purchases: List[PurchaseRecord]; sales: List[SaleRecord]
    opening_inventory: float = 0; opening_qty: int = 0

class InventoryValuationResponse(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()) if 'uuid' in dir() else "n/a")
    company_id: str; method: str
    total_goods_available: float; total_units_available: int
    total_units_sold: int; cost_of_goods_sold: float
    ending_inventory: float; ending_qty: int
    total_sales_revenue: float; gross_profit: float; gross_margin: float
...
async def calculate_inventory(req: InventoryValuationRequest):
    ...
    return InventoryValuationResponse(
        company_id=req.company_id, method=req.method,
        total_goods_available=round(total_cost, 2), total_units_available=total_units,
        total_units_sold=units_sold, cost_of_goods_sold=round(cogs, 2),
        ending_inventory=round(ending_inv, 2), ending_qty=ending_qty,
