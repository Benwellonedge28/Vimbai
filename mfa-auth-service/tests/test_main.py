...
def test_mfa_backup_restore_with_recovery_phrase():
    response = client.post(
        "/auth/verify",
        json={"user_id": "u1", "action_type": "restore_backup", "knowledge_factor": "recovery_phrase_12_words"},
    )
    assert response.status_code == 200
    assert response.json()["authorized"] == True
...
def test_mfa_backup_restore_insufficient_factors():
    response = client.post(
        "/auth/verify", json={"user_id": "u1", "action_type": "restore_backup", "knowledge_factor": "password_hash"}
    )
...
def test_mfa_approve_expense_success():
    response = client.post(
        "/auth/verify", json={"user_id": "u1", "action_type": "approve_expense", "biometric_token": "bio_token"}
    )
    assert response.status_code == 200
    assert response.json()["authorized"] == True
