...
async def add_cost(job_id: str, entry: JobCostEntry):
    ...
    for jobs in _jobs.values():
        for j in jobs:
            if j.id == job_id:
                if entry.cost_type == "materials":
                    j.materials_cost += entry.amount
                elif entry.cost_type == "labor":
                    j.labor_cost += entry.amount
                elif entry.cost_type == "overhead":
                    j.overhead_cost += entry.amount
                elif entry.cost_type == "subcontractor":
                    j.subcontractor_cost += entry.amount
