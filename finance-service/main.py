...
try:
    ...
    TRACER = setup_tracing(service_name="finance-service", instrument_app=app)
except ImportError:
    TRACER = None
    import logging
...
@app.post(
    "/budgets/",
    response_model=models.BudgetInDB,
...
async def get_budget_by_id(budget_id: str, db_session: AsyncSession = Depends(get_db_session)):
...
@app.get(
    "/budgets/",
    response_model=List[models.BudgetInDB],
    dependencies=[Depends(check_permission("finance.read.budgets"))],
)
async def get_all_budgets(user_id: str = Depends(get_user_id), db_session: AsyncSession = Depends(get_db_session)):
...
async def update_budget(
    budget_id: str, budget_update: models.BudgetUpdate, db_session: AsyncSession = Depends(get_db_session)
):
...
async def delete_budget(budget_id: str, db_session: AsyncSession = Depends(get_db_session)):
...
@app.post(
    "/forecasts/baseline",
    response_model=models.ForecastInDB,
    status_code=status.HTTP_201_CREATED,
...
@app.post(
    "/forecasts/{baseline_forecast_id}/scenario",
    response_model=models.ForecastInDB,
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(check_permission("finance.write.forecasts"))],
)
...
async def get_forecast_by_id(forecast_id: str, db_session: AsyncSession = Depends(get_db_session)):
...
@app.get(
    "/forecasts/",
    response_model=List[models.ForecastInDB],
    dependencies=[Depends(check_permission("finance.read.forecasts"))],
)
async def get_all_forecasts(user_id: str = Depends(get_user_id), db_session: AsyncSession = Depends(get_db_session)):
...
async def compare_forecast_scenarios(forecast_ids: List[str], db_session: AsyncSession = Depends(get_db_session)):
    scenario_engine = ScenarioEngine(db_session)
...
async def update_forecast(
    forecast_id: str, forecast_update: models.ForecastUpdate, db_session: AsyncSession = Depends(get_db_session)
):
...
async def delete_forecast(forecast_id: str, db_session: AsyncSession = Depends(get_db_session)):
