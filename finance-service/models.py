...
from datetime import datetime, date # NEW: Import date for scenario parameters
...
class FinancialForecastBase(BaseModel):
    ...
    forecast_type: Literal["revenue", "expenses", "profit", "cash_flow", "custom"] = Field(..., description="Type of financial metric being forecasted.")
    methodology: Optional[str] = Field(None, description="Methodology used for forecasting (e.g., 'historical_average', 'regression', 'AI_model').")
...
class ScenarioParameter(BaseModel):
    ...
    param_type: Literal["percentage", "absolute_amount", "multiplier"] = Field(..., description="Type of the parameter value.")
...
class ScenarioBase(BaseModel):
    name: str = Field(..., min_length=3, max_length=100, description="Name of the scenario (e.g., 'Best Case Growth', 'Economic Downturn').")
...
class ScenarioAnalysisResult(BaseModel):
    ...
    projected_metrics: Dict[str, Any] = Field({}, description="Key projected financial metrics (e.g., 'total_revenue': 12345.67).")
    projected_data_points: List[FinancialForecastDataPoint] = Field([], description="The detailed forecast generated by the scenario.")
