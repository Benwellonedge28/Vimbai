...
if "risk_assessment_service" not in _sys.modules or not hasattr(
    _sys.modules.get("risk_assessment_service"), "__path__"
):
    _spec = importlib.util.spec_from_file_location("risk_assessment_service", _os.path.join(_HERE, "__init__.py"))
    _pkg = importlib.util.module_from_spec(_spec)
    _spec.loader.exec_module(_pkg)
    _sys.modules["risk_assessment_service"] = _pkg
    _sys.modules["risk_assessment_service"].__path__ = [_HERE]
...
from risk_assessment_service import crud, models
from risk_assessment_service.dependencies import book_id_var, get_db_session, get_user_id
from risk_assessment_service.exceptions import NotFoundError
from risk_assessment_service.models import RiskCategory, RiskLevel
...
try:
    ...
    TRACER = setup_tracing(service_name="risk-assessment-service", instrument_app=app)
except ImportError:
    TRACER = None
