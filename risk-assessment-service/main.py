"""Vimbai Risk Assessment Service - Risk management and investigation. Port: 8330"""
import os, uuid
...
app = FastAPI(title="Vimbai Risk Assessment Service", version="2.0.0", docs_url="/docs")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])
try:
    from shared.tracing import setup_tracing; TRACER = setup_tracing(service_name="risk-assessment-service", instrument_app=app)
except ImportError:
    TRACER = None

class RiskCategory(str, Enum):
    FINANCIAL = "financial"; OPERATIONAL = "operational"; STRATEGIC = "strategic"; COMPLIANCE = "compliance"; CYBER = "cyber"; MARKET = "market"; CREDIT = "credit"; LIQUIDITY = "liquidity"
...
async def get_risks(company_id: str, category: Optional[str] = None, level: Optional[str] = None):
    ...
    if category: risks = [r for r in risks if r.category.value == category]
    if level: risks = [r for r in risks if r.level.value == level]
    return {"company_id": company_id, "risks": risks, "total": len(risks), "by_level": {l: sum(1 for r in risks if r.level.value == l) for l in RiskLevel}}

@app.put("/risks/{risk_id}")
async def update_risk(risk_id: str, likelihood: Optional[int] = None, impact: Optional[int] = None, mitigation: Optional[str] = None, status: Optional[str] = None):
    for risks in _risks.values():
        for r in risks:
            if r.id == risk_id:
                if likelihood is not None: r.likelihood = likelihood
                if impact is not None: r.impact = impact
                if mitigation is not None: r.mitigation = mitigation
...
async def risk_dashboard(company_id: str):
    risks = _risks.get(company_id, [])
    if not risks:
        return {"company_id": company_id, "total_risks": 0, "by_level": {}, "by_category": {}, "avg_score": 0, "top_risks": []}
    ...
    return {"company_id": company_id, "total_risks": len(risks), "by_level": by_level, "by_category": by_category, "avg_score": avg, "top_risks": top}
