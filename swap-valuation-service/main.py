...
async def value_swaps(request: SwapValuationRequest):
    ...
    for swap in request.swaps:
        payments_per_year = (
            2 if swap.payment_frequency == "semi-annual" else 4 if swap.payment_frequency == "quarterly" else 1
        )

        discount_factor = sum(
            1 / ((1 + request.ois_rate) ** (i / payments_per_year))
            for i in range(1, swap.tenor_years * payments_per_year + 1)
        )
