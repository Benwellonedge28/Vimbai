...
async def root():
    return {
        ...
        "description": "Accounting Rate of Return (ARR) calculation",
    }


@app.post("/calculate")
async def calculate_arr(initial_investment: float, residual_value: float, total_profits: float, years: int):
    """
...
async def calculate_arr_from_annual_profits(
    annual_profits: List[float], initial_investment: float, residual_value: float = 0
):
...
async def calculate_average_profit(total_profits: float, years: int):
    ...
    return {"total_profits": total_profits, "years": years, "average_annual_profit": round(avg, 2)}
...
async def compare_arr_projects(
    projects: List[
        dict
    ],  # [{"name": "A", "initial_investment": x, "residual_value": y, "annual_profits": [p1, p2, ...]}]
):
...
@app.post("/decision")
async def arr_decision(arr: float, target_return: float):
