...
try:
    ...
    TRACER = setup_tracing(service_name="webhook-service", instrument_app=app)
except ImportError:
    TRACER = None
...
async def get_endpoints(company_id: str):
    return {
        ...
        "endpoints": _endpoints.get(company_id, []),
        "total": len(_endpoints.get(company_id, [])),
    }
...
async def get_deliveries(endpoint_id: str, limit: int = 50):
    return {
        ...
        "deliveries": _deliveries.get(endpoint_id, [])[-limit:],
        "total": len(_deliveries.get(endpoint_id, [])),
    }
