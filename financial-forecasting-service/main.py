...
class ForecastRequest(BaseModel):
    company_id: str; metric_name: str
...
async def forecast(req: ForecastRequest):
    ...
    if n < 3:
        return ForecastResult(
            company_id=req.company_id, metric_name=req.metric_name,
            forecast=[values[-1]] * req.periods_ahead if values else [0] * req.periods_ahead,
            method_used="naive", confidence_interval_lower=[0] * req.periods_ahead,
            confidence_interval_upper=[0] * req.periods_ahead,
            trend="insufficient_data", growth_rate=0, r_squared=0
        )
    ...
    if method == "linear":
        ...
        last = smoothed[-1]
        forecast = [last * (1 + (smoothed[-1] - smoothed[0]) / max(abs(smoothed[0]), 1) * (i + 1) / n) for i in range(req.periods_ahead)]
        r2 = 0
    ...
    return ForecastResult(
        ...
        trend=trend, growth_rate=round(growth_rate, 2),
