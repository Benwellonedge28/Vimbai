"""
Vimbai Discounted Payback Period Service
...
app = FastAPI(title="Vimbai Discounted Payback Period Service", version=SERVICE_VERSION, docs_url="/docs")
app.add_middleware(
...
@app.post("/calculate")
async def calculate_discounted_payback(initial_investment: float, cash_flows: List[float], discount_rate: float):
    """
...
async def calculate_discounted_payback_percent(
    initial_investment: float, cash_flows: List[float], discount_rate_percent: float
):
    ...
    for i, cf in enumerate(cash_flows, 1):
        ...
        schedule.append({"year": i, "cash_flow": cf, "discount_factor": round(df, 4), "present_value": round(pv, 2)})
...
@app.post("/compare")
async def compare_discounted_payback(projects: List[dict]):
    ...
    for proj in projects:
        ...
        results.append({"name": name, "discounted_payback_period": round(payback, 2) if payback else None})
