...
from fastapi import Depends, Request, status  # Removed HTTPException, use custom ones
from fastapi.security import OAuth2PasswordBearer
from invoicing_service.exceptions import ForbiddenError, UnauthorizedError  # NEW
...
async def get_current_user_claims(token: str = Depends(oauth2_scheme)):
    credentials_exception = UnauthorizedError(
        detail="Could not validate credentials", code="INVALID_CREDENTIALS"
    )  # MODIFIED
    try:
        ...
        return {"user_id": user_id, "username": username, "role": user_role, "permissions": permissions}
    except JWTError:
        raise credentials_exception
    ...
    def permission_checker(claims: dict = Depends(get_current_user_claims)):
        ...
        if required_permission not in user_permissions:
            raise ForbiddenError(
                detail=f"Not enough permissions. Requires: {required_permission}", code="INSUFFICIENT_PERMISSIONS"
            )  # MODIFIED
