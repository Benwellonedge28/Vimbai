...
@app.post(
    "/customers/",
    response_model=models.CustomerInDB,
...
@app.get(
    "/customers/",
    response_model=List[models.CustomerInDB],
    dependencies=[Depends(check_permission("invoicing.read.customers"))],
)
async def read_all_customers(user_id: str = Depends(get_user_id), db_session: AsyncSession = Depends(get_db_session)):
...
async def read_customer_by_id(
    customer_id: str, user_id: str = Depends(get_user_id), db_session: AsyncSession = Depends(get_db_session)
):
...
@app.post(
    "/invoices/",
    response_model=models.InvoiceInDB,
    status_code=status.HTTP_201_CREATED,
...
@app.get(
    "/invoices/",
    response_model=List[models.InvoiceInDB],
    dependencies=[Depends(check_permission("invoicing.read.invoices"))],
)
async def read_all_invoices(user_id: str = Depends(get_user_id), db_session: AsyncSession = Depends(get_db_session)):
...
async def read_invoice_by_number(
    invoice_number: str, user_id: str = Depends(get_user_id), db_session: AsyncSession = Depends(get_db_session)
):
...
@app.post(
    "/invoices/{invoice_number}/record-payment",
    response_model=models.CreateJournalEntryResponse,
...
async def record_invoice_payment(
    ...
    payment_date: datetime = Query(
        default_factory=datetime.utcnow, description="Date of the payment (ISO format)"
    ),  # Modified to be a Query parameter
    user_id: str = Depends(get_user_id),
    jwt_token: str = Depends(get_jwt_token),
    db_session: AsyncSession = Depends(get_db_session),
):
    try:
        return await crud.record_payment_for_invoice(
            db_session, invoice_number, user_id, payment_amount, payment_date, jwt_token
        )
