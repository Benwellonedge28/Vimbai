...
class CustomerBase(BaseModel):
    name: str = Field(
        ..., min_length=3, max_length=100, description="Customer's name or company name."
    )  # ADDED VALIDATION
    email: str = Field(
        ..., example="contact@acmecorp.com", description="Customer's primary contact email."
    )  # ADDED VALIDATION
...
class InvoiceItemBase(BaseModel):
    description: str = Field(
        ..., min_length=3, max_length=500, description="Description of the service or product."
    )  # ADDED VALIDATION
    quantity: condecimal(decimal_places=2, ge=Decimal("0.00")) = Field(
        Decimal("1.00"), description="Quantity of the item."
    )  # ADDED VALIDATION
    unit_price: condecimal(decimal_places=2, ge=Decimal("0.00")) = Field(
        ..., description="Unit price of the item."
    )  # ADDED VALIDATION
    amount: condecimal(decimal_places=2, ge=Decimal("0.00")) = Field(
        ...
        if quantity is not None and unit_price is not None:
            ...
            if v != calculated_amount:
                ...
                if abs(v - calculated_amount) > Decimal("0.01"):
                    raise ValueError(
                        f"Amount ({v}) does not match quantity ({quantity}) * unit_price ({unit_price}) = {calculated_amount}"
                    )
        return v
...
class InvoiceBase(BaseModel):
    ...
    status: Literal["draft", "outstanding", "paid", "overdue", "void"] = Field(
        "draft", description="Current status of the invoice."
    )  # ADDED VALIDATION (Literal)
    notes: Optional[str] = Field(
        None, max_length=1000, description="Any additional notes for the invoice."
    )  # ADDED VALIDATION
