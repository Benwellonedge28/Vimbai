...
async def get_customer_by_id(session: AsyncSession, customer_id: str, user_id: str) -> Optional[CustomerInDB]:
    ...
    RETURN c
    """
    result = await session.run(query, customer_id=customer_id, user_id=user_id)
    record = await result.single()
    if record:
        node = record["c"]
        return CustomerInDB(
...
async def update_customer(session: AsyncSession, customer_id: str, user_id: str, customer_data: CustomerUpdate) -> Optional[CustomerInDB]:
    ...
    params = {"customer_id": customer_id, "user_id": user_id, **update_fields}
    result = await session.run(query, params)
    record = await result.single()
...
async def delete_customer(session: AsyncSession, customer_id: str, user_id: str) -> bool:
    # Delete Customer node and all its invoices
    query = """
    MATCH (c:Customer {customer_id: $customer_id, user_id: $user_id})
    OPTIONAL MATCH (c)-[:HAS_INVOICE]->(i:Invoice)-[:HAS_ITEM]->(ii:InvoiceItem)
    DETACH DELETE c, i, ii
    """
    result = await session.run(query, customer_id=customer_id, user_id=user_id)
...
async def get_invoice_by_number(session: AsyncSession, invoice_number: str, user_id: str) -> Optional[InvoiceInDB]:
    ...
    if record:
        ...
        for item_node in items_data:
            ...
            customer_query = "MATCH (c:Customer)-[:HAS_INVOICE]->(i:Invoice {id: $invoice_id}) RETURN c.customer_id AS customer_id"
            customer_result = await session.run(customer_query, invoice_id=invoice_node["id"])
            customer_id = (await customer_result.single())["customer_id"]
...
async def get_all_invoices(session: AsyncSession, user_id: str) -> List[InvoiceInDB]:
    ...
    async def record_payment_for_invoice(session: AsyncSession, invoice_number: str, user_id: str, payment_amount: Decimal, payment_date: datetime, jwt_token: str) -> CreateJournalEntryResponse:
        ...
        if invoice.status == "paid":
            return CreateJournalEntryResponse(status="failed", message="Invoice already marked as paid.")
