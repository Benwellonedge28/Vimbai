...
SAMPLE_FAMILY_GROUP = {
    ...
    "group_type": "family",
    "members": [{"user_id": "parent_001", "role": "parent"}, {"user_id": "child_001", "role": "child"}],
...
def test_split_expense():
    client.post("/groups", json=SAMPLE_FAMILY_GROUP)
    response = client.post("/groups/family_001/expenses/split?total_amount=120.0", json=["parent_001", "child_001"])
    assert response.status_code == 200
    data = response.json()
