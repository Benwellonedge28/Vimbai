...
class Segment(BaseModel):
    ...
    name: str; segment_type: str  # business, geographical
    revenue: float; expenses: float; assets: float; liabilities: float = 0
    profit_or_loss: float = 0; identifiable: bool = True

class SegmentReportRequest(BaseModel):
    company_id: str; fiscal_year: int; segments: List[Segment]
    reconciliation_revenue: float = 0; reconciliation_expenses: float = 0

class SegmentReportResult(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; fiscal_year: int
    reportable_segments: int; total_revenue: float; total_profit: float
    total_assets: float
    segments: List[Dict]
    reconciliation: Dict
...
async def generate_report(req: SegmentReportRequest):
    ...
    for s in req.segments:
        ...
        segments_detail.append({
            ...
            "type": s.segment_type, "revenue": round(s.revenue, 2),
            "expenses": round(s.expenses, 2), "profit_or_loss": round(s.profit_or_loss, 2),
            "assets": round(s.assets, 2), "liabilities": round(s.liabilities, 2),
            "revenue_pct": round(rev_pct, 1), "is_reportable": is_reportable
        })
