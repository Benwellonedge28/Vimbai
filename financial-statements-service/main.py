...
async def prepare_financial_statements(request: FinancialStatementsRequest):
    ...
    total_liabilities = request.balance_sheet.get("current_liabilities", 0) + request.balance_sheet.get("non_current_liabilities", 0)
    ...
    quick_ratio = (request.balance_sheet.get("current_assets", 0) - request.balance_sheet.get("inventory", 0)) / request.balance_sheet.get("current_liabilities", 1)
    ...
    health_score = min(100, (current_ratio / 2 * 25) + (profit_margin / 20 * 25) + ((100 - abs(key_ratios["debt_to_equity"])) * 0.25) + (key_ratios["roe"] / 30 * 25))
