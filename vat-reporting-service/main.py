...
class VATTransaction(BaseModel):
    description: str; amount: float; vat_rate: float = VAT_STANDARD_RATE
    transaction_type: str = "standard"  # standard, zero_rated, exempt

class VATReturnRequest(BaseModel):
    company_id: str; tax_period: str  # e.g. "2026-Q1"
...
class VATReturn(BaseModel):
    ...
    company_id: str; tax_period: str
    total_output_vat: float; total_input_vat: float
    net_vat_payable: float; net_vat_refundable: float
    standard_rated_sales: float; zero_rated_sales: float; exempt_sales: float
...
async def prepare_vat_return(req: VATReturnRequest):
    total_output_vat = 0; total_input_vat = 0
    std_sales = 0; zero_sales = 0; exempt_sales = 0; std_purchases = 0
    
    for tx in req.output_transactions:
        vat = tx.amount * tx.vat_rate
        total_output_vat += vat
        if tx.transaction_type == "standard": std_sales += tx.amount
        elif tx.transaction_type == "zero_rated": zero_sales += tx.amount
        else: exempt_sales += tx.amount
    
    for tx in req.input_transactions:
        vat = tx.amount * tx.vat_rate
        total_input_vat += vat
        if tx.transaction_type == "standard": std_purchases += tx.amount
    ...
    return VATReturn(
        company_id=req.company_id, tax_period=req.tax_period,
