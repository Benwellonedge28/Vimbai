...
try:
    from shared.tracing import setup_tracing; TRACER = setup_tracing(service_name="zero-based-budgeting-service", instrument_app=app)
except ImportError:
    TRACER = None
...
async def get_packages(company_id: str, department: Optional[str] = None, status_filter: Optional[str] = None):
    ...
    if department: pkgs = [p for p in pkgs if p.department == department]
    if status_filter: pkgs = [p for p in pkgs if p.status.value == status_filter]
...
async def update_status(package_id: str, status: ZBBStatus, reviewer: str = "", notes: str = ""):
    for pkgs in _packages.values():
        for p in pkgs:
            if p.id == package_id:
                p.status = status
                if reviewer: p.reviewer = reviewer
                if notes: p.review_notes = notes
...
async def zbb_summary(company_id: str):
    ...
    return {"company_id": company_id, "total_packages": len(pkgs), "total_budget": total, "by_department": dict(by_dept), "by_status": dict(by_status)}
