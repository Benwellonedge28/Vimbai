...
class ThroughputRequest(BaseModel):
    ...
    available_constraint_minutes: int = 480
...
async def analyze_throughput(req: ThroughputRequest):
    ...
    for p in req.products:
        ...
        products.append(
            {
                "name": p.name,
                "throughput_per_unit": round(throughput_per_unit, 2),
        ...
        optimal_mix.append(
            {
                ...
                "produce": produce,
                "demand": p["demand"],
                "throughput_contribution": round(contribution, 2),
            }
        )
    ...
    utilization = (
        (req.available_constraint_minutes - remaining_minutes) / req.available_constraint_minutes * 100
        if req.available_constraint_minutes
        else 0
    )
