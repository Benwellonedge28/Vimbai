...
class WorkflowStep(BaseModel):
    ...
    step_type: Literal["approval", "notification", "action", "conditional", "parallel"] = Field(
        ..., description="Type of workflow step."
    )
    assignee_role: Optional[str] = Field(
        None, description="Role required to complete this step (e.g., 'Accountant', 'FinanceManager')."
    )
    ...
    config: Dict[str, Any] = Field(
        {}, description="JSON configuration for the step (e.g., notification message, action details)."
    )
    next_steps: List[str] = Field([], description="List of step_ids for next steps to execute on completion.")
    on_rejection_steps: List[str] = Field(
        [], description="List of step_ids for steps to execute on rejection (for approval steps)."
    )


class WorkflowDefinitionBase(BaseModel):
    name: str = Field(
        ..., min_length=3, max_length=100, description="Name of the workflow definition (e.g., 'Invoice Approval')."
    )
    description: Optional[str] = Field(None, max_length=500, description="Description of the workflow.")
    trigger_event: str = Field(
        ..., description="Event that triggers this workflow (e.g., 'JournalEntryCreated', 'VendorBillReceived')."
    )
