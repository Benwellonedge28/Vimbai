...
from workflow_service.exceptions import NotFoundError
...
class WorkflowEngine:
    ...
    async def _evaluate_next_steps(
        self,
        instance: models.WorkflowInstanceInDB,
        definition: models.WorkflowDefinitionInDB,
        completed_step_id: str,
        action: Optional[str] = None,
    ) -> List[models.WorkflowStep]:
    ...
    async def start_workflow_instance(
        self, definition_id: str, triggered_by_event: Dict[str, Any], context: Dict[str, Any]
    ) -> models.WorkflowInstanceInDB:
        """
        ...
        updated_instance = await crud.update_workflow_instance(
            self.db_session,
            instance.id,
            models.WorkflowInstanceUpdate(current_step_ids=instance.current_step_ids, tasks=instance.tasks),
        )
    ...
    async def complete_workflow_task(
        self, instance_id: str, task_completion_data: models.WorkflowTaskCompletion
    ) -> models.WorkflowInstanceInDB:
        """
        ...
        if not definition:
            raise NotFoundError(
                f"Workflow Definition {instance.workflow_definition_id} not found for instance {instance_id}."
            )
        ...
        if task_status_to_complete.status != "pending" and task_status_to_complete.status != "in_progress":
            raise WorkflowEngineException(
                f"Workflow Task {task_completion_data.task_id} is already {task_status_to_complete.status}."
            )
        ...
        # Evaluate next steps
        next_steps_defs = await self._evaluate_next_steps(
            instance, definition, task_status_to_complete.step_id, task_completion_data.action
        )
        ...
        if not instance.current_step_ids:
            ...
            instance.status != "failed" and instance.status != "cancelled"
        ):  # Ensure status isn't overwritten if already failed/cancelled
