...
try:
    ...
    TRACER = setup_tracing(service_name="workflow-service", instrument_app=app)
except ImportError:
    TRACER = None
    import logging
...
@app.post(
    ...
    response_model=models.WorkflowDefinitionInDB,
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(check_permission("workflow.write.definitions"))],
)
async def create_workflow_definition(
    definition: models.WorkflowDefinitionCreate, db_session: AsyncSession = Depends(get_db_session)
):
...
@app.get(
    "/workflow-definitions/",
    response_model=List[models.WorkflowDefinitionInDB],
    dependencies=[Depends(check_permission("workflow.read.definitions"))],
)
async def read_all_workflow_definitions(db_session: AsyncSession = Depends(get_db_session)):
...
async def read_workflow_definition_by_id(definition_id: str, db_session: AsyncSession = Depends(get_db_session)):
...
async def update_workflow_definition(
    definition_id: str, definition: models.WorkflowDefinitionUpdate, db_session: AsyncSession = Depends(get_db_session)
):
...
async def delete_workflow_definition(definition_id: str, db_session: AsyncSession = Depends(get_db_session)):
...
@app.post(
    "/workflow-instances/",
    response_model=models.WorkflowInstanceInDB,
    status_code=status.HTTP_201_CREATED,
...
async def read_workflow_instance_by_id(instance_id: str, db_session: AsyncSession = Depends(get_db_session)):
...
async def update_workflow_instance(
    instance_id: str, instance: models.WorkflowInstanceUpdate, db_session: AsyncSession = Depends(get_db_session)
):
...
async def delete_workflow_instance(instance_id: str, db_session: AsyncSession = Depends(get_db_session)):
