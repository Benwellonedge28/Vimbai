...
class InstrumentType(str, Enum):
    LETTER_OF_CREDIT = "letter_of_credit"; DOCUMENTARY_COLLECTION = "documentary_collection"
    BANK_GUARANTEE = "bank_guarantee"; ADVANCE_PAYMENT = "advance_payment"; FACTORING = "factoring"

class TradeInstrument(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; instrument_type: InstrumentType
    counterparty: str; amount: float; currency: str = "USD"
    ...
    issuing_bank: str = ""; confirming_bank: str = ""

class InstrumentResult(BaseModel):
    id: str; company_id: str; instrument_type: str
    amount: float; fee_estimate: float; status: str
    risk_assessment: str; documentation_required: List[str] = []

_instruments: Dict[str, List[TradeInstrument]] = {}

def _estimate_fee(itype: InstrumentType, amount: float) -> float:
    rates = {InstrumentType.LETTER_OF_CREDIT: 0.002, InstrumentType.DOCUMENTARY_COLLECTION: 0.001,
             InstrumentType.BANK_GUARANTEE: 0.015, InstrumentType.ADVANCE_PAYMENT: 0.0,
...
async def create_instrument(inst: TradeInstrument):
    ...
    return InstrumentResult(
        id=inst.id, company_id=inst.company_id,
        instrument_type=inst.instrument_type.value, amount=inst.amount,
        fee_estimate=round(fee, 2), status=inst.status,
        risk_assessment=risk, documentation_required=docs
    )
...
async def present_documents(company_id: str, instrument_id: str):
    ...
    from fastapi import HTTPException; raise HTTPException(status_code=404, detail="Instrument not found")
