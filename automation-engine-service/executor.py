...
async def execute_task_instance(db_session: AsyncSession, instance_id: str):
    ...
    if not definition:
        print(
            f"Executor: Automation Task Definition {instance.task_definition_id} not found for instance {instance_id}."
        )
        await crud.update_automation_task_instance(
            db_session,
            instance_id,
            models.AutomationTaskInstanceUpdate(
                status="failed", end_time=datetime.now(timezone.utc), error_message="Task Definition not found."
            ),
        )
        return
    ...
    await crud.create_automation_log(
        db_session,
        models.AutomationLogCreate(
    ...
    try:
        ...
        if not service_base_url:
            raise ValueError(
                f"Environment variable {service_url_env_var} not set for target service {definition.service_target}."
            )
        ...
        async with httpx.AsyncClient() as client:
            ...
            await crud.update_automation_task_instance(
                ...
                instance_id,
                models.AutomationTaskInstanceUpdate(
                    status="completed", end_time=datetime.now(timezone.utc), output=response_output
                ),
            )

    except httpx.HTTPStatusError as http_error:
        error_message = (
            f"HTTP Error during task execution: {http_error.response.status_code} - {http_error.response.text}"
        )
        ...
        await crud.update_automation_task_instance(
            ...
            instance_id,
            models.AutomationTaskInstanceUpdate(
                status="failed", end_time=datetime.now(timezone.utc), error_message=error_message
            ),
        )
        ...
        await crud.create_automation_log(
            db_session,
            models.AutomationLogCreate(
                instance_id=instance_id, log_level="ERROR", message=error_message, details={"exception": str(e)}
            ),
        )
        await crud.update_automation_task_instance(
            db_session,
