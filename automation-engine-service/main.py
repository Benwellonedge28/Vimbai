...
class TriggerType(str, Enum):
    SCHEDULED = "scheduled"; EVENT = "event"; MANUAL = "manual"; WEBHOOK = "webhook"

class WorkflowStatus(str, Enum):
    PENDING = "pending"; RUNNING = "running"; COMPLETED = "completed"; FAILED = "failed"; PAUSED = "paused"

class WorkflowStep(BaseModel):
    step_id: str; step_name: str; action: str
    params: Dict[str, Any] = {}; depends_on: List[str] = []; timeout_seconds: int = 300

class AutomationRule(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    name: str; company_id: str
    trigger: TriggerType; condition: Dict[str, Any] = {}
    steps: List[WorkflowStep] = []
    enabled: bool = True; priority: int = 5

class WorkflowExecution(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    rule_id: str; company_id: str; status: WorkflowStatus
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": SERVICE_NAME, "version": "2.0.0", "rules": len(_rules), "executions": len(_executions)}
...
async def toggle_rule(rule_id: str):
    ...
    from fastapi import HTTPException; raise HTTPException(status_code=404, detail="Rule not found")
...
async def execute_rule(rule_id: str, background_tasks: BackgroundTasks):
    ...
    rule = _rules[rule_id]
    if not rule.enabled:
        from fastapi import HTTPException; raise HTTPException(status_code=400, detail="Rule is disabled")
    
    execution = WorkflowExecution(
        rule_id=rule_id, company_id=rule.company_id, status=WorkflowStatus.RUNNING
    )
...
async def _run_workflow(execution_id: str, rule: AutomationRule):
    ...
    for step in rule.steps:
        ...
        result = {"step_id": step.step_id, "step_name": step.step_name, "action": step.action, "status": "completed", "params": step.params}
...
async def get_execution(execution_id: str):
    if execution_id not in _executions:
        from fastapi import HTTPException; raise HTTPException(status_code=404, detail="Execution not found")
