        ...
        for definition in active_definitions:
            ...
            if definition.schedule_type == "cron" and definition.cron_schedule:
                try:
                    ...
                    base_time = definition.last_executed_at or now - timedelta(
                        minutes=1
                    )  # Look back a bit if never run
                    itr = croniter.croniter(definition.cron_schedule, base_time)
                    next_run = itr.get_next(datetime)

                    if next_run <= now and (
                        definition.last_executed_at is None or next_run > definition.last_executed_at
                    ):
                        trigger_now = True
                ...
                if (
                    definition.last_executed_at is None
                    or (now - definition.last_executed_at).total_seconds() >= definition.interval_seconds
                ):
                    trigger_now = True
                    definition.next_execution_at = now + timedelta(
                        seconds=definition.interval_seconds
                    )  # Update next_execution_at based on interval
            ...
            if trigger_now:
                ...
                instance_create = models.AutomationTaskInstanceCreate(
                    task_definition_id=definition.id, triggered_by="schedule", start_time=now
                )
