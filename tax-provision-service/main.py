...
class TemporaryDifference(BaseModel):
    description: str; book_amount: float; tax_amount: float
    type: str = "taxable"  # taxable, deductible

class TaxProvisionRequest(BaseModel):
    company_id: str; fiscal_year: int
    pre_tax_income: float; statutory_rate: float = 0.25
    permanent_differences: float = 0
    temp_differences: List[TemporaryDifference] = []
    loss_carryforward: float = 0; valuation_allowance: float = 0

class TaxProvisionResult(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; fiscal_year: int
    current_tax_expense: float; deferred_tax_expense: float
    total_tax_expense: float; effective_rate: float
    deferred_tax_assets: float; deferred_tax_liabilities: float
    net_deferred_tax: float; valuation_allowance: float
