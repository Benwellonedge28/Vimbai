...
async def create_audit_sample(request: AuditSamplingRequest):
    ...
    if request.parameters.sampling_method == "attribute":
        ...
        n = (
            pop_size
            * (request.parameters.expected_error_rate * (1 - request.parameters.expected_error_rate))
            / (conf_factor**2 * request.parameters.tolerable_error_rate**2)
        )
    ...
    return AuditSamplingResponse(
        ...
        stratification_levels={
            "high": len([s for s in sample_items if s.risk_score >= 70]),
