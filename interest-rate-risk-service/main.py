...
async def analyze_interest_rate_risk(request: InterestRateRiskRequest):
    ...
    for bucket in buckets:
        ...
        liability_gap = sum(
            item.balance for item in request.rate_sensitive_liabilities if item.repricing_bucket == bucket
        )
    ...
    risk_rating = (
        "LOW"
        if abs(nii_impact / total_assets) < 0.05
        else "MEDIUM" if abs(nii_impact / total_assets) < 0.10 else "HIGH"
    )
