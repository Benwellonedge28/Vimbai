...
class TaxRiskItem(BaseModel):
    ...
    description: str; risk_type: str  # transfer_pricing, vat_gap, permanent_establishment, treaty, reporting
    potential_exposure: float; probability: float = 0.5
    jurisdiction: str = "ZW"; mitigation_actions: List[str] = []

class TaxRiskRequest(BaseModel):
    company_id: str; risks: List[TaxRiskItem]

class TaxRiskAssessment(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; assessment_date: str
    overall_risk_level: RiskLevel; total_exposure: float
    weighted_exposure: float; risk_count: int
    risks: List[Dict]; recommendations: List[str] = []
...
async def assess_risk(req: TaxRiskRequest):
    ...
    if weighted > 500000: overall = RiskLevel.CRITICAL
    elif weighted > 100000: overall = RiskLevel.HIGH
    elif weighted > 25000: overall = RiskLevel.MEDIUM
    else: overall = RiskLevel.LOW
    
    risks_detail = []
    for r in req.risks:
        risk_score = r.potential_exposure * r.probability
        level = RiskLevel.CRITICAL if risk_score > 250000 else RiskLevel.HIGH if risk_score > 50000 else RiskLevel.MEDIUM if risk_score > 10000 else RiskLevel.LOW
        risks_detail.append({
            "risk_id": r.risk_id, "description": r.description,
            "risk_type": r.risk_type, "potential_exposure": r.potential_exposure,
            "probability": r.probability, "weighted_score": round(risk_score, 2),
            "level": level.value, "jurisdiction": r.jurisdiction,
    ...
    return TaxRiskAssessment(
        ...
        risks=risks_detail, recommendations=recommendations
    )
