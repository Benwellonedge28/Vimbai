...
class BenchmarkData(BaseModel):
    industry_avg_roe: float = 0.15
    industry_avg_current_ratio: float = 1.8
    industry_avg_debt_to_equity: float = 0.6
    industry_avg_net_margin: float = 0.10
...
async def analyze(req: AnalysisRequest):
    ...
    liquidity = {
        ...
        "quick_ratio": (
            round((f.current_assets - f.inventory) / f.current_liabilities, 2) if f.current_liabilities else 0
        ),
    ...
    benchmark = {
        "roe": {
            ...
            "industry": req.benchmark.industry_avg_roe * 100,
            "above_industry": profitability["roe"] > req.benchmark.industry_avg_roe * 100,
        },
        "current_ratio": {
            "company": liquidity["current_ratio"],
            "industry": req.benchmark.industry_avg_current_ratio,
            "above_industry": liquidity["current_ratio"] > req.benchmark.industry_avg_current_ratio,
        },
        "debt_to_equity": {
            "company": solvency["debt_to_equity"],
            "industry": req.benchmark.industry_avg_debt_to_equity,
            "above_industry": solvency["debt_to_equity"] < req.benchmark.industry_avg_debt_to_equity,
        },
        "net_margin": {
            "company": profitability["net_margin"],
            "industry": req.benchmark.industry_avg_net_margin * 100,
