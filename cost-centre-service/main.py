...
app = FastAPI(title="FinAcc Cost Centre Service", version=SERVICE_VERSION, docs_url="/docs")
...
async def create_cost_centre(
    centre_code: str, centre_name: str, centre_type: str,
    department_id: Optional[str] = None, floor_area: float = 0,
    number_of_personnel: int = 0, machine_value: float = 0,
    is_service_centre: bool = False, parent_centre_id: Optional[str] = None
):
    """Create a cost centre."""
    centre = CostCentre(
        centre_code=centre_code, centre_name=centre_name,
        centre_type=centre_type, department_id=department_id,
        floor_area=floor_area, number_of_personnel=number_of_personnel,
        machine_value=machine_value, is_service_centre=is_service_centre,
...
async def update_cost_centre(
    centre_id: str, floor_area: Optional[float] = None,
...
async def add_cost_centre_cost(
    cost_centre_id: str, cost_code: str, cost_description: str,
    amount: float, cost_type: str, period: str
):
    """Add cost to a cost centre."""
    cost = CostCentreCost(
        cost_centre_id=cost_centre_id, cost_code=cost_code,
        cost_description=cost_description, amount=amount,
...
async def generate_cost_centre_summary(
    cost_centre_id: str, period: str,
    allocated_service_costs: float = 0
):
    """Generate cost centre summary."""
    costs = [c for c in cost_centre_costs
             if c.cost_centre_id == cost_centre_id and c.period == period]

    summary = CostCentreSummary(
        cost_centre_id=cost_centre_id, period=period,
        allocated_service_costs=allocated_service_costs
    )
    ...
    summary.total_cost = (
        summary.total_direct_costs + summary.total_indirect_costs +
        summary.total_overhead + allocated_service_costs
    )
...
async def get_cost_centre(centre_id: str):
    ...
    return {
        "cost_centre": centre,
        "costs": centre_costs,
        "total_costs": sum(c.amount for c in centre_costs)
    }
