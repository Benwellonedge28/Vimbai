...
class FakeSession:
    ...
    async def run(self, query, params=None, **kw):
        ...
        var, label, found = self._match_nodes(query, merged)
        om = re.search(r"ORDER BY (\w+)\.(\w+)( ASC| DESC)?", query)
        if om:

            def _order_key(t):
                v = t[1]["props"].get(om.group(2))
                return v.iso() if hasattr(v, "iso") else str(v)

            direction = (om.group(3) or " ASC").strip()
            found.sort(key=_order_key, reverse=(direction == "DESC"))
