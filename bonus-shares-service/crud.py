"""
Bonus Shares Service CRUD Operations

Neo4j-backed persistence driven generically by the pydantic models:
bonus issues map to labeled nodes with user ownership (X-User-Id) and
...
from bonus_shares_service.dependencies import book_id_var
from bonus_shares_service.exceptions import NotFoundError, ValidationError
from bonus_shares_service.models import BonusIssue
...
async def create_bonus_issue(session, user_id, issue: BonusIssue) -> BonusIssue:
    return await _create(session, user_id, "BonusIssueNode", "OWNS_BONUS_ISSUE", BonusIssue, issue)


async def list_bonus_issues(session, user_id) -> List[BonusIssue]:
    return await _list(session, user_id, "BonusIssueNode", "OWNS_BONUS_ISSUE", BonusIssue)
