...
class PartnerBuyoutRequest(BaseModel):
    company_id: str
    outgoing_partner_id: str
    outgoing_partner_name: str
    capital_balance: float
    share_of_goodwill: float = 0
    share_of_reserves: float = 0
    loan_to_partner: float = 0
    agreed_payment: float
    remaining_partners: List[Dict] = []


class BuyoutResult(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str
    outgoing_partner: str
    capital_due: float
    goodwill_due: float
    reserves_due: float
    loan_due: float
...
async def calculate_retirement(req: PartnerBuyoutRequest):
    ...
    return BuyoutResult(
        ...
        outgoing_partner=req.outgoing_partner_name,
        capital_due=round(req.capital_balance, 2),
        goodwill_due=round(req.share_of_goodwill, 2),
        reserves_due=round(req.share_of_reserves, 2),
        loan_due=round(req.loan_to_partner, 2),
        total_payable=round(total_payable, 2),
        agreed_payment=round(req.agreed_payment, 2),
        difference=round(difference, 2),
        gain_or_loss_on_retirement=round(gain_or_loss, 2),
