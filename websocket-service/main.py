...
try:
    from shared.tracing import setup_tracing; TRACER = setup_tracing(service_name="websocket-service", instrument_app=app)
except ImportError:
    TRACER = None
...
class ConnectionManager:
    ...
    async def connect(self, ws: WebSocket, channel: str):
        ...
        await ws.send_json({"type": "connected", "channel": channel, "timestamp": datetime.now(timezone.utc).isoformat()})
...
@app.get("/")
async def health(): return {"status": "healthy", "service": SERVICE_NAME, "active_channels": manager.stats()}

@app.get("/stats")
async def stats():
    return {"connections": manager.stats(), "total_connections": sum(len(v) for v in manager.active.values()), "messages_sent": len(_message_log)}
...
async def websocket_endpoint(ws: WebSocket, channel: str):
    ...
    try:
        ...
        await manager.broadcast(channel, {"type": "disconnect", "channel": channel, "timestamp": datetime.now(timezone.utc).isoformat()})

@app.post("/broadcast/{channel}")
async def broadcast_to_channel(channel: str, message: dict):
    msg = {"channel": channel, "data": message, "timestamp": datetime.now(timezone.utc).isoformat(), "type": "broadcast"}
    _message_log.append(msg)
    await manager.broadcast(channel, msg)
