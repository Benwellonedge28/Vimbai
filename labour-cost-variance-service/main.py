...
class LabourData(BaseModel):
    department: str; standard_rate: float; actual_rate: float
    standard_hours: float; actual_hours: float; standard_output: float = 0; actual_output: float = 0

class LabourVarianceRequest(BaseModel):
    company_id: str; period: str; departments: List[LabourData]

class LabourVarianceResult(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; period: str
    total_rate_variance: float; total_efficiency_variance: float; total_cost_variance: float
...
async def analyze_labour_variance(req: LabourVarianceRequest):
    total_rate = 0; total_eff = 0; total_cost = 0; total_idle = 0
    dept_results = []
    ...
    for dept in req.departments:
        ...
        total_rate += rate_variance; total_eff += efficiency_variance
        ...
        dept_results.append({
            ...
            "standard_hours": dept.standard_hours, "actual_hours": dept.actual_hours,
            "standard_rate": dept.standard_rate, "actual_rate": dept.actual_rate,
            "favorable_rate": rate_variance < 0, "favorable_efficiency": efficiency_variance < 0
        })
    ...
    return LabourVarianceResult(
        ...
        total_idle_time_variance=0, departments=dept_results
    )
