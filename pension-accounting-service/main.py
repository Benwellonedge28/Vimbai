...
class PensionRequest(BaseModel):
    company_id: str; fiscal_year: int; plan_name: str
    obligation_beginning: float; service_cost: float
    interest_cost: float; benefits_paid: float
    actuarial_gain_loss: float = 0; past_service_cost: float = 0
    plan_assets_beginning: float; contributions: float
    expected_return: float; actual_return: float = 0
    benefits_paid_from_assets: float = 0

class PensionResult(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; plan_name: str; fiscal_year: int
    dbo_end: float; plan_assets_end: float
    funded_position: float; net_defined_benefit_liability: float
    current_service_cost: float; interest_cost: float
    net_interest: float; remeasurements: float
    total_pension_expense: float
    oci_actuarial_gain_loss: float; oci_return_adjustment: float = 0
...
async def calculate_pension(req: PensionRequest):
    dbo_end = req.obligation_beginning + req.service_cost + req.interest_cost + \
              req.actuarial_gain_loss + req.past_service_cost - req.benefits_paid
    ...
    return PensionResult(
        company_id=req.company_id, plan_name=req.plan_name, fiscal_year=req.fiscal_year,
        dbo_end=round(dbo_end, 2), plan_assets_end=round(plan_assets_end, 2),
