...
class DerivativeRequest(BaseModel):
    ...
    barrier_type: Optional[str] = None  # knock_in, knock_out
    is_call: bool = True
...
def black_scholes(S, K, T, r, sigma, is_call=True):
    ...
    if is_call:
        price = S * 0.5 * (1 + math.erf(d1 / math.sqrt(2))) - K * math.exp(-r * T) * 0.5 * (
            1 + math.erf(d2 / math.sqrt(2))
        )
        delta = 0.5 * (1 + math.erf(d1 / math.sqrt(2)))
    else:
        price = K * math.exp(-r * T) * 0.5 * (1 - math.erf(d2 / math.sqrt(2))) - S * 0.5 * (
            1 - math.erf(d1 / math.sqrt(2))
        )
        delta = -0.5 * (1 - math.erf(d1 / math.sqrt(2)))
...
async def price_derivative(req: DerivativeRequest):
    ...
    theta = (
        -S * n_prime_d1 * sigma / (2 * math.sqrt(T))
        - r
        * K
        * math.exp(-r * T)
        * (0.5 * (1 + math.erf(d2 / math.sqrt(2))) if req.is_call else 0.5 * (1 - math.erf(d2 / math.sqrt(2))))
    ) / 365
    rho = (
        K
        * T
    ...
    if req.option_type == "asian":
        ...
        if req.barrier_price:
            barrier_prob = 0.5 * (
                1
                + math.erf(
                    (math.log(req.barrier_price / S) + (r - 0.5 * sigma**2) * T) / (sigma * math.sqrt(T) * math.sqrt(2))
                )
            )
