...
if "investigation_service" not in _sys.modules or not hasattr(_sys.modules.get("investigation_service"), "__path__"):
    _spec = importlib.util.spec_from_file_location("investigation_service", _os.path.join(_HERE, "__init__.py"))
    _pkg = importlib.util.module_from_spec(_spec)
    _spec.loader.exec_module(_pkg)
    _sys.modules["investigation_service"] = _pkg
    _sys.modules["investigation_service"].__path__ = [_HERE]
...
from investigation_service import crud, models
from investigation_service.dependencies import book_id_var, get_db_session, get_user_id
from investigation_service.exceptions import NotFoundError
from investigation_service.models import RiskCategory, RiskLevel
...
try:
    ...
    TRACER = setup_tracing(service_name="investigation-service", instrument_app=app)
except ImportError:
    TRACER = None
