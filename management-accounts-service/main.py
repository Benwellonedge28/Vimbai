...
async def prepare_management_accounts(request: ManagementAccountsRequest):
    ...
    pnl_statement = {
        ...
        "gross_margin": round((total_revenue - request.cost_items.get("cogs", 0)) / total_revenue * 100, 2) if total_revenue else 0
    }
    
    total_assets = sum(request.balance_sheet_items.get(k, 0) for k in ["current_assets", "non_current_assets"])
    total_liabilities = sum(request.balance_sheet_items.get(k, 0) for k in ["current_liabilities", "non_current_liabilities"])
