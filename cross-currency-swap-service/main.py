...
async def value_cross_currency_swaps(request: CrossCurrencySwapRequest):
    ...
    for swap in request.swaps:
        ...
        pv_interest_1 = swap.notional_1 * swap.rate_1 * sum(math.exp(-rate_1 * t) for t in [1, 2, 3, 4, 5] if t <= swap.tenor_years) if swap.tenor_years <= 5 else swap.notional_1 * swap.rate_1 / rate_1 * (1 - math.exp(-rate_1 * swap.tenor_years))
        pv_interest_2 = swap.notional_2 * swap.rate_2 * sum(math.exp(-rate_2 * t) for t in [1, 2, 3, 4, 5] if t <= swap.tenor_years) if swap.tenor_years <= 5 else swap.notional_2 * swap.rate_2 / rate_2 * (1 - math.exp(-rate_2 * swap.tenor_years))
