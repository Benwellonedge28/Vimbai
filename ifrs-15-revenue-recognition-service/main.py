...
async def recognize_revenue(request: RevenueRecognitionRequest):
    ...
    for contract in request.contracts:
        ...
        step1_analysis.append(Step1Identification(
        ...
        distinct_services = [po.description for po in request.performance_obligations if po.obligation_id in contract.performance_obligation_ids]
        step2_list.append(Step2Obligations(
            contract_id=contract.contract_id,
        ...
        total_ssp = sum(po.standalone_selling_price for po in request.performance_obligations if po.obligation_id in contract.performance_obligation_ids)
        allocations = {}
        ...
        step3_allocations.append(Step3PriceAllocation(
            contract_id=contract.contract_id,
            transaction_price=contract.transaction_price,
            allocations=allocations,
            variable_consideration_allocated=contract.variable_consideration,
            remaining_obligations=[po_id for po_id in contract.performance_obligation_ids if po_id not in request.actual_progress]
        ))
        ...
        for po in request.performance_obligations:
            if po.obligation_id in contract.performance_obligation_ids:
                ...
                step4_satisfaction.append(Step4Satisfaction(
        ...
        contract_positions.append(ContractAssetLiability(
            contract_id=contract.contract_id,
