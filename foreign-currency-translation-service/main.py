...
class BalanceSheetItem(BaseModel):
    account: str; amount: float; is_monetary: bool = True

class TranslationRequest(BaseModel):
    company_id: str; subsidiary: str
    functional_currency: str; presentation_currency: str
    closing_rate: float; avg_rate: float; historical_rate: float
...
async def translate_financials(req: TranslationRequest):
    translated_assets = 0; translated_income = 0
    details = []
    ...
    for item in req.net_assets:
        ...
        details.append({
            "account": item.account, "original": round(item.amount, 2),
            "rate_used": rate, "rate_type": "closing" if item.is_monetary else "historical",
    ...
    for item in req.income_statement:
        ...
        details.append({
            "account": item.account, "original": round(item.amount, 2),
            "rate_used": req.avg_rate, "rate_type": "average",
            "translated": round(translated, 2)
        })
    
    goodwill_translated = req.goodwill * req.closing_rate
    exchange_diff = (sum(i.amount for i in req.net_assets if i.is_monetary) * req.closing_rate -
                     sum(i.amount for i in req.net_assets if i.is_monetary) * req.avg_rate)
    cta = exchange_diff
    
    return TranslationResult(
        company_id=req.company_id, subsidiary=req.subsidiary,
