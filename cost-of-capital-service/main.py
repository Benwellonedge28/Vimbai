"""
Vimbai Cost of Capital Service
...
app = FastAPI(title="Vimbai Cost of Capital Service", version=SERVICE_VERSION, docs_url="/docs")
app.add_middleware(
...
async def root():
    return {
        ...
        "description": "Cost of capital/rate of interest calculations",
    }


@app.post("/calculate")
async def calculate_cost_of_capital(interest_rate: float, risk_premium: float = 0):
...
async def cost_of_capital_percent(interest_rate_percent: float, risk_premium_percent: float = 0):
...
async def calculate_wacc(
    equity_value: float, cost_of_equity: float, debt_value: float, cost_of_debt: float, tax_rate: float = 0
):
...
async def simple_rate(principal: float, amount_after: float, years: int):
...
async def compound_rate(present_value: float, future_value: float, years: int):
