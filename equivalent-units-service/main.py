...
class EquivalentUnitsRequest(BaseModel):
    ...
    units_started: int
    units_completed: int
    ending_wip_units: int
    ending_wip_completion_materials: float = 0.0
    ending_wip_completion_conversion: float = 0.0
    beginning_wip_units: int = 0
    beginning_wip_completion_materials: float = 0.0
    beginning_wip_completion_conversion: float = 0.0
    materials_cost_beginning: float = 0
    conversion_cost_beginning: float = 0
    materials_cost_added: float = 0
    conversion_cost_added: float = 0
...
class EquivalentUnitsResult(BaseModel):
    ...
    period: str
    method: str
    equivalent_units_materials: float
    equivalent_units_conversion: float
    cost_per_unit_materials: float
    cost_per_unit_conversion: float
    cost_per_unit_total: float
    cost_of_completed: float
    cost_of_ending_wip: float
...
async def calculate_equivalent_units(req: EquivalentUnitsRequest):
    if req.method == "weighted_average":
        ...
        eu_materials = (
            req.units_completed - req.beginning_wip_units * req.beginning_wip_completion_materials
        ) + req.ending_wip_units * req.ending_wip_completion_materials
        eu_conversion = (
            req.units_completed - req.beginning_wip_units * req.beginning_wip_completion_conversion
