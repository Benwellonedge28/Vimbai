...
class Supplier(BaseModel):
    ...
    name: str; contact: str = ""; lead_time_days: int = 7
    rating: float = 5.0; products: List[str] = []

class InventoryItem(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    sku: str; name: str; company_id: str
    quantity: int = 0; reorder_point: int = 10; reorder_qty: int = 50
    unit_cost: float = 0; unit_price: float = 0
    supplier_id: Optional[str] = None; lead_time_days: int = 7

class PurchaseOrder(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; supplier_id: str; item_sku: str; quantity: int
    unit_cost: float; status: str = "pending"  # pending, approved, shipped, received
    order_date: str = Field(default_factory=lambda: datetime.now(timezone.utc).isoformat())
    expected_delivery: Optional[str] = None

class DemandForecast(BaseModel):
    sku: str; company_id: str
    historical_data: List[float] = []  # units sold per period
    forecast_periods: int = 3

class ForecastResult(BaseModel):
    sku: str; forecast: List[float]; method: str; confidence: float
    reorder_recommended: bool; recommended_qty: int = 0
    ...
    for item in items:
        if item.quantity <= item.reorder_point:
            ...
            low.append({
                ...
                "reorder_qty": item.reorder_qty, "supplier_id": item.supplier_id,
...
async def receive_po(po_id: str, company_id: str):
    ...
    from fastapi import HTTPException; raise HTTPException(status_code=404, detail="PO not found")

@app.post("/forecast", response_model=ForecastResult)
async def forecast_demand(req: DemandForecast):
    if len(req.historical_data) < 2:
        return ForecastResult(sku=req.sku, forecast=[0]*req.forecast_periods,
                              method="insufficient_data", confidence=0, reorder_recommended=False)
    ...
    return ForecastResult(sku=req.sku, forecast=[round(f, 1) for f in forecast],
                          method="moving_average_with_trend", confidence=round(confidence, 2),
                          reorder_recommended=reorder, recommended_qty=rec_qty)
