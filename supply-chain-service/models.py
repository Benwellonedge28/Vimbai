...
class PurchaseOrderItemBase(BaseModel):
    ...
    unit_price: condecimal(decimal_places=2, ge=Decimal('0.00')) = Field(..., description="Agreed unit price for this item on the PO.")
    line_total: condecimal(decimal_places=2, ge=Decimal('0.00')) = Field(..., description="Calculated total for this line item (quantity * unit_price).")
...
class PurchaseOrderBase(BaseModel):
    ...
    total_amount: condecimal(decimal_places=2, ge=Decimal('0.00')) = Field(..., description="Total amount of the purchase order.")
    currency: str = Field("USD", max_length=3, description="Currency of the purchase order.")
    status: Literal["draft", "pending_approval", "approved", "ordered", "received", "cancelled", "partially_received"] = Field("draft", description="Current status of the purchase order.")
    notes: Optional[str] = Field(None, max_length=1000, description="Any notes or special instructions for the supplier.")
    items: List[PurchaseOrderItemBase] = Field(..., min_length=1, description="List of items included in this purchase order.")
...
class PurchaseOrderUpdate(BaseModel):
    ...
    status: Optional[Literal["draft", "pending_approval", "approved", "ordered", "received", "cancelled", "partially_received"]] = None
...
class SalesInvoiceBase(BaseModel):
    ...
    total_amount: condecimal(decimal_places=2, ge=Decimal('0.00')) = Field(..., description="Total amount of the invoice.")
    currency: str = Field("USD", max_length=3, description="Currency of the invoice.")
    status: Literal["draft", "sent", "paid", "overdue", "void"] = Field("draft", description="Current status of the invoice.")
