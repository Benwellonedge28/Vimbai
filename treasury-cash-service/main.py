...
async def forecast_cash_flow(request: CashForecastRequest):
    ...
    scenarios = {
        ...
        "optimistic": [
            {
                "date": p.date,
                "opening_balance": p.opening_balance * 1.1,
                "expected_inflows": p.expected_inflows * 1.2,
                "expected_outflows": p.expected_outflows * 0.9,
                "closing_balance": p.closing_balance * 1.15,
                "net_change": p.net_change * 1.3,
            }
        ...
        "pessimistic": [
            {
                "date": p.date,
                "opening_balance": p.opening_balance * 0.9,
                "expected_inflows": p.expected_inflows * 0.8,
                "expected_outflows": p.expected_outflows * 1.1,
                "closing_balance": p.closing_balance * 0.85,
                "net_change": p.net_change * 0.7,
            }
            for p in projections
        ],
    }
...
async def optimize_cash_flow(request: CashFlowOptimizationRequest):
    ...
    return CashFlowOptimizationResponse(
        ...
        maturity_schedule=[
            {"date": (date.today() + timedelta(days=m["maturity"])).isoformat(), "amount": m["amount"]}
            for m in investments
        ],
    )
...
async def reconcile_bank(request: BankReconciliationRequest):
    ...
    deposits_in_transit = [{"deposit_id": "D001", "amount": 12000.0, "date": "2024-01-28"}]
...
async def calculate_liquidity_ratios(request: LiquidityRatioRequest):
    ...
    cash_ratio = (
        (request.cash_and_equivalents + request.marketable_securities) / request.current_liabilities
        if request.current_liabilities
        else 0
    )

    daily_cash_burn = 50000.0
    defensive_interval = (
        (request.cash_and_equivalents + request.marketable_securities) / daily_cash_burn if daily_cash_burn else 0
    )
