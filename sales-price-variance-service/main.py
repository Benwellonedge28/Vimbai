...
class VarianceResult(BaseModel):
    ...
    company_id: str
    period: str
    total_sales_price_variance: float
    total_sales_volume_variance: float
    total_sales_mix_variance: float
    total_sales_quantity_variance: float
    items: List[Dict] = []
...
async def analyze_variance(req: VarianceRequest):
    ...
    avg_budget_price = (
        sum(i.budgeted_price * i.budgeted_volume for i in req.items) / total_budget_volume if total_budget_volume else 0
    )
