...
Port: 8366
"""
import os, uuid, time, hashlib, json, base64
...
class SSOAuthRequest(BaseModel):
    organization_id: str; idp_token: str
    provider: str = "oidc"  # oidc, saml, azure_ad, okta

class SSOAuthResponse(BaseModel):
    vimbai_access_token: str; user_id: str; organization_id: str
    expires_at: str; message: str; provider: str

class SSOConfigRequest(BaseModel):
    organization_id: str; provider: str
    client_id: str = ""; client_secret: str = ""
    issuer_url: str = ""; saml_metadata_url: str = ""
...
def _generate_vimbai_token(user_id: str, org_id: str, provider: str) -> dict:
    ...
    _session_store[token] = {
        "user_id": user_id, "org_id": org_id, "provider": provider,
...
async def authenticate_via_sso(req: SSOAuthRequest):
    ...
    return SSOAuthResponse(
        ...
        user_id=user_id, organization_id=req.organization_id,
        expires_at=token_info["expires_at"],
        message="SSO authentication successful", provider=req.provider
    )
