...
from banking_integration_service.clients.accounting_service_client import (  # NEW
    ...
    AccountingServiceClientException,
    MockJournalEntryCreate,
    MockJournalLineCreate,
)
...
class TransactionProcessor:
    ...
    async def _apply_categorization_rules(
        self, user_id: str, transaction: models.BankTransactionInDB
    ...
    async def process_new_bank_transaction(
        self, user_id: str, transaction: models.BankTransactionInDB
        ...
        if categorization_update.category and categorization_update.category != transaction.category:
            transaction = await crud.update_bank_transaction(self.db_session, transaction.id, categorization_update)
            transaction = await crud.get_bank_transaction(
                self.db_session, transaction.id
        ...
        if not transaction.is_reconciled and not transaction.vimbai_journal_entry_id:
            ...
            for rule in rules:
                if (
                    rule.is_active
                    and rule.target_vimbai_account_number
                    and (
                        (
                            rule.match_field == "description"
                            and rule.match_pattern.lower() in transaction.description.lower()
                        )
                        or (
                            rule.match_field == "payee"
                            and rule.match_pattern.lower() in transaction.description.lower()
                        )
                    )
                ):
            ...
            journal_entry_data = MockJournalEntryCreate(  # Using Mock class, should be actual JournalEntryCreate
                ...
                lines=[
                    MockJournalLineCreate(
                        ...
                        debit=float(abs(transaction.amount)),
                        credit=float(Decimal("0.00")),
                        description=description_je,
                    ),
                    MockJournalLineCreate(
                        account_number=credit_account,
                        debit=float(Decimal("0.00")),
                        credit=float(abs(transaction.amount)),
                        description=description_je,
                    ),
                ],
            )
            ...
            try:
                ...
                transaction = await crud.update_bank_transaction(
                    self.db_session, transaction.id, models.BankTransactionUpdate(vimbai_journal_entry_id=new_je_id)
                )
                ...
                print(
                    f"ERROR: An unexpected error occurred while creating journal entry for transaction {transaction.id}: {e}"
                )

        # 3. Attempt to find reconciliation match
        if transaction.vimbai_journal_entry_id and not transaction.is_reconciled:
            transaction = await crud.update_bank_transaction(
                self.db_session, transaction.id, models.BankTransactionUpdate(is_reconciled=True)
            )
