...
class BankAccountBase(BaseModel):
    ...
    subtype: str = Field(
        ..., description="Account subtype (e.g., checking, savings, commercial credit card).", min_length=1
    )
    currency: str = Field(..., max_length=3, description="ISO 4217 currency code (e.g., USD, ZAR).", example="USD")
    current_balance: condecimal(max_digits=18, decimal_places=2) = Field(
        ..., description="Current balance of the account."
    )
    available_balance: Optional[condecimal(max_digits=18, decimal_places=2)] = Field(
        None, description="Available balance (if different from current)."
    )
    vimbai_account_number: Optional[str] = Field(
        None, description="Corresponding account number in Vimbai Accounting Service."
    )
...
class BankTransactionBase(BaseModel):
    ...
    amount: condecimal(max_digits=18, decimal_places=2) = Field(
        ..., description="Transaction amount (positive for debit, negative for credit).", multiple_of=Decimal("0.01")
    )
...
class TransactionCategorizationRuleBase(BaseModel):
    ...
    target_vimbai_account_number: Optional[str] = Field(
        None, description="Optional: Vimbai account number to suggest for JE."
    )
    is_active: bool = True
