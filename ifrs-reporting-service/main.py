...
async def generate_ifrs_report(request: IFRSReportingRequest):
    ...
    ifrs_metrics = {
        "basic_eps": round(request.financial_data.get("ifrs_net_income", 0) / request.financial_data.get("shares", 1), 4),
        "diluted_eps": round(request.financial_data.get("ifrs_net_income", 0) / request.financial_data.get("diluted_shares", 1), 4),
        "tangible_assets": round(request.financial_data.get("ifrs_assets", {}).get("total", 0) - request.financial_data.get("ifrs_intangibles", 0), 2)
    }
    
    compliance_check = {
        "standards_applied": request.ifrs_standards,
