...
async def analyze_liquidity(request: LiquidityRequest):
    ...
    quick_ratio = (
        (request.cash + request.marketable_securities) / request.current_liabilities
    ...
    utilization = (
        (request.credit_facilities_total - request.credit_facilities_available) / request.credit_facilities_total
        if request.credit_facilities_total
        else 0
    )
    ...
    stress_scenarios = {
        ...
        "payment_delay_60": {
            ...
            "runway_days": (
                (total_liquid / (request.monthly_min_revenue * 0.6)) * 30 if request.monthly_min_revenue else 0
            ),
        },
    }
