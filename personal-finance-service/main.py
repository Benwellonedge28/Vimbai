...
try:
    from shared.tracing import setup_tracing; TRACER = setup_tracing(service_name="personal-finance-service", instrument_app=app)
except ImportError:
    TRACER = None
...
async def get_goals(user_id: str):
    ...
    return {"user_id": user_id, "goals": goals, "total": len(goals), "progress_avg": sum(g.current_amount / max(1, g.target_amount) for g in goals) / max(1, len(goals))}
...
async def financial_overview(user_id: str):
    ...
    goals = _goals.get(user_id, [])
    return {"user_id": user_id, "monthly_income": income_total, "total_debt": debt_total, "debt_to_income": debt_total / max(1, income_total * 12) * 100, "active_goals": len(goals), "goals_progress": sum(g.current_amount for g in goals), "goals_target": sum(g.target_amount for g in goals)}
