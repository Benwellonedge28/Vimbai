...
class GrantType(str, Enum):
    ASSET = "asset"; INCOME = "income"

class GrantRecognition(str, Enum):
    DEFERRED_INCOME = "deferred_income"; NET_ASSET = "net_asset"

class GrantRequest(BaseModel):
    company_id: str; grant_name: str; granting_authority: str = ""
    grant_amount: float; grant_type: GrantType
...
class GrantResult(BaseModel):
    ...
    company_id: str; grant_name: str; granting_authority: str
    grant_amount: float; grant_type: str; recognition_method: str
    annual_recognition: float; deferred_income_balance: float
    annual_amortization: float = 0; journal_entries: List[str] = []; disclosure_notes: List[str] = []
    conditions: List[str] = []; compliance_status: str
