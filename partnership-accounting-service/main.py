...
class Partner(BaseModel):
    ...
    salary: float = 0
    interest_on_capital_rate: float = 0.05
    drawings: float = 0
...
class PartnershipRequest(BaseModel):
    ...
    net_profit: float
    interest_rate_on_capital: float = 0.05
...
class PartnershipResult(BaseModel):
    ...
    total_profit_share: float
    residual_profit: float
    ...
    for p in req.partners:
        ...
        accounts.append(
            PartnerAccount(
                ...
                name=p.name,
                opening_capital=p.capital_contribution,
                salary=p.salary,
