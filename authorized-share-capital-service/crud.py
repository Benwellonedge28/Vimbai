"""
Authorized Share Capital Service CRUD Operations

Neo4j-backed persistence driven generically by the pydantic models:
share classes, issuances, and buybacks each map to a labeled node type
with user ownership (X-User-Id) and book_id stamping. Every read applies
the Book filter `WHERE ($book_id IS NULL OR x.book_id = $book_id)`.

Record-keeping only: this service records share capital movements; it
never moves money. Corrections use reversing entries.
"""
...
from authorized_share_capital_service.dependencies import book_id_var
from authorized_share_capital_service.exceptions import NotFoundError, ValidationError
from authorized_share_capital_service.models import ShareBuyback, ShareClass, ShareIssuance
...
async def create_share_class(session, user_id, share_class: ShareClass) -> ShareClass:
    return await _create(session, user_id, "ShareClassNode", "OWNS_SHARE_CLASS", ShareClass, share_class)


async def get_share_class(session, user_id, class_id: str) -> Optional[ShareClass]:
    return await _get(session, user_id, "ShareClassNode", "OWNS_SHARE_CLASS", ShareClass, class_id)


async def list_share_classes(session, user_id) -> List[ShareClass]:
    return await _list(session, user_id, "ShareClassNode", "OWNS_SHARE_CLASS", ShareClass)


async def save_share_class(session, user_id, share_class: ShareClass) -> None:
    await _save(session, user_id, "ShareClassNode", "OWNS_SHARE_CLASS", ShareClass, share_class)


# ============================================================================
# Store: Share Issuances
# ============================================================================


async def create_issuance(session, user_id, issuance: ShareIssuance) -> ShareIssuance:
    return await _create(session, user_id, "ShareIssuanceNode", "OWNS_SHARE_ISSUANCE", ShareIssuance, issuance)


async def list_issuances(session, user_id) -> List[ShareIssuance]:
    return await _list(session, user_id, "ShareIssuanceNode", "OWNS_SHARE_ISSUANCE", ShareIssuance)


# ============================================================================
# Store: Share Buybacks
# ============================================================================


async def create_buyback(session, user_id, buyback: ShareBuyback) -> ShareBuyback:
    return await _create(session, user_id, "ShareBuybackNode", "OWNS_SHARE_BUYBACK", ShareBuyback, buyback)


async def list_buybacks(session, user_id) -> List[ShareBuyback]:
    return await _list(session, user_id, "ShareBuybackNode", "OWNS_SHARE_BUYBACK", ShareBuyback)
