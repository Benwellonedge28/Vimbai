...
async def analyze_credit_derivatives(request: CreditDerivativesRequest):
    ...
    total_cva = sum(
        pos.notional * (1 - request.recovery_rate) * 0.01 * math.exp(-request.risk_free_rate * pos.maturity_years)
        for pos in request.positions
    )
    ...
    portfolio_exposure = {
        ...
        "avg_maturity": (
            round(sum(p.maturity_years for p in request.positions) / len(request.positions), 2)
