"""
Issued Share Capital Service CRUD Operations

Neo4j-backed persistence driven generically by the pydantic models:
shareholders and share issues map to labeled node types with user
ownership (X-User-Id) and book_id stamping. Every read applies the Book
...
from issued_share_capital_service.dependencies import book_id_var
from issued_share_capital_service.exceptions import NotFoundError, ValidationError
from issued_share_capital_service.models import Shareholder, ShareIssue
...
async def create_shareholder(session, user_id, holder: Shareholder) -> Shareholder:
    return await _create(session, user_id, "ShareholderNode", "OWNS_SHAREHOLDER", Shareholder, holder)


async def list_shareholders(session, user_id) -> List[Shareholder]:
    return await _list(session, user_id, "ShareholderNode", "OWNS_SHAREHOLDER", Shareholder)
...
async def create_share_issue(session, user_id, issue: ShareIssue) -> ShareIssue:
    return await _create(session, user_id, "ShareIssueNode", "OWNS_SHARE_ISSUE", ShareIssue, issue)


async def list_share_issues(session, user_id) -> List[ShareIssue]:
    return await _list(session, user_id, "ShareIssueNode", "OWNS_SHARE_ISSUE", ShareIssue)
