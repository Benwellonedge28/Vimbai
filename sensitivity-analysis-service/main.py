...
try:
    from shared.tracing import setup_tracing; TRACER = setup_tracing(service_name="sensitivity-analysis-service", instrument_app=app)
except ImportError:
    TRACER = None
...
async def run_analysis(req: AnalysisRequest):
    ...
    for var in req.variables:
        for step in req.change_steps:
            ...
            elasticity = (step / 100) / ((impact - req.base_target_value) / max(1, req.base_target_value)) if impact != req.base_target_value else 0
            results.append(SensitivityResult(
                variable_name=var.name, base_value=var.base_value, changed_value=changed_value,
                change_pct=step, impact_on_target=impact, elasticity=abs(elasticity)
            ))
    ...
    resp = AnalysisResponse(company_id=req.company_id, target_metric=req.target_metric, base_target_value=req.base_target_value, results=results, most_sensitive_variable=most_sensitive)
    _analyses[req.company_id].append(resp)
    return resp

@app.get("/analyses/{company_id}")
async def get_analyses(company_id: str):
    return {"company_id": company_id, "analyses": _analyses.get(company_id, []), "total": len(_analyses.get(company_id, []))}
