...
class TestBusinessCombinationService:
    ...
    def test_acquisition(self):
        resp = self.client.post("/acquire", json={
            "company_id": "comp-1", "acquirer": "Parent Co", "acquiree": "Sub Co",
            "purchase_price": 500000, "identifiable_assets": [
            ...
            "contingent_consideration": 20000, "acquisition_costs": 5000
        })
        data = resp.json()
...
class TestDivestitureService:
    ...
    def test_disposal(self):
        resp = self.client.post("/dispose", json={
            "company_id": "comp-1", "subsidiary_name": "Sub Co",
            "carrying_value": 300000, "disposal_proceeds": 400000,
            "disposal_costs": 10000, "associated_goodwill": 50000
        })
        data = resp.json()
        assert resp.status_code == 200
...
class TestForeignCurrencyTranslationService:
    ...
    def test_translation(self):
        resp = self.client.post("/translate", json={
            "company_id": "comp-1", "subsidiary": "Zambia Sub",
            "functional_currency": "ZMW", "presentation_currency": "USD",
            "closing_rate": 0.05, "avg_rate": 0.048, "historical_rate": 0.055,
            "net_assets": [
                {"account": "Cash", "amount": 1000000, "is_monetary": True},
                {"account": "Property", "amount": 5000000, "is_monetary": False}
            ],
            "income_statement": [
                {"account": "Revenue", "amount": 2000000, "is_monetary": True}
            ],
            "goodwill": 500000
        })
        data = resp.json()
...
class TestAnalyticsService:
    ...
    def test_full_analysis(self):
        resp = self.client.post("/analyze", json={
            ...
            "financials": {
                "revenue": 2000000, "net_income": 300000, "total_assets": 1500000,
                "current_assets": 500000, "current_liabilities": 300000,
                "total_liabilities": 600000, "total_equity": 900000,
                "inventory": 100000, "accounts_receivable": 200000,
                "cogs": 1200000, "operating_cash_flow": 400000, "shares_outstanding": 10000
            }
        })
        data = resp.json()
        assert resp.status_code == 200
    ...
    def test_scorecard(self):
        resp = self.client.post("/score", json={
            ...
            "kpis": [
                ...
                {"name": "Customer Satisfaction", "perspective": "customer", "target": 90, "actual": 85, "weight": 1.0},
                {"name": "Process Efficiency", "perspective": "internal", "target": 95, "actual": 92, "weight": 1.0},
                {"name": "Training Hours", "perspective": "learning_growth", "target": 40, "actual": 30, "weight": 1.0}
            ]
        })
        data = resp.json()
        assert resp.status_code == 200
...
class TestBankFeeAnalysisService:
    ...
    def test_fee_analysis(self):
        resp = self.client.post("/analyze", json={
            "company_id": "comp-1", "period": "2026", "bank_name": "Stanbic",
            "charges": [
                {"date": "2026-01-15", "description": "Monthly maintenance", "amount": 50, "category": "maintenance"},
                ...
                {"date": "2026-02-10", "description": "FX conversion", "amount": 100, "category": "foreign_exchange"}
            ],
            "transaction_volume": 100, "avg_balance": 50000
        })
        data = resp.json()
        assert resp.status_code == 200
...
class TestFinancialForecastingService:
    ...
    def test_linear_forecast(self):
        resp = self.client.post("/forecast", json={
            "company_id": "comp-1", "metric_name": "Revenue",
            "historical_values": [100, 110, 120, 130, 140, 150],
            "periods_ahead": 4, "method": "linear"
        })
        data = resp.json()
        assert resp.status_code == 200
    ...
    def test_insufficient_data(self):
        resp = self.client.post("/forecast", json={
            "company_id": "comp-1", "metric_name": "Revenue",
            "historical_values": [100], "periods_ahead": 4
        })
        data = resp.json()
...
class TestPensionAccountingService:
    ...
    def test_pension_calc(self):
        resp = self.client.post("/calculate", json={
            "company_id": "comp-1", "fiscal_year": 2026, "plan_name": "Staff Pension",
            "obligation_beginning": 1000000, "service_cost": 50000,
            "interest_cost": 70000, "benefits_paid": 80000,
            "actuarial_gain_loss": -20000,
            "plan_assets_beginning": 800000, "contributions": 60000,
            "expected_return": 56000, "actual_return": 50000,
...
class TestCapitalAllocationService:
    ...
    def test_allocation(self):
        resp = self.client.post("/allocate", json={
            "company_id": "comp-1", "capital_budget": 500000,
            "projects": [
                {"name": "Project A", "initial_investment": 200000,
                 "annual_cash_flows": [80000, 80000, 80000, 80000], "discount_rate": 0.10, "strategic_value": 8},
                {"name": "Project B", "initial_investment": 300000,
                 "annual_cash_flows": [120000, 120000, 120000], "discount_rate": 0.10, "strategic_value": 6},
                {"name": "Project C", "initial_investment": 400000,
                 "annual_cash_flows": [150000, 150000, 150000], "discount_rate": 0.10, "strategic_value": 9}
            ]
        })
        data = resp.json()
        assert resp.status_code == 200
...
class TestInternalControlsService:
    ...
    def test_control_testing(self):
        resp = self.client.post("/test", json={
            ...
            "controls": [
                {"name": "Segregation of Duties", "control_type": "preventive",
                 "description": "Segregation between authorization and custody",
                 "process": "Procurement", "test_sample_size": 25, "exceptions_found": 0},
                {"name": "Bank Reconciliation", "control_type": "detective",
                 "description": "Monthly bank reconciliation review",
                 "process": "Treasury", "test_sample_size": 12, "exceptions_found": 2},
                {"name": "Access Controls", "control_type": "preventive",
                 "description": "User access review", "process": "IT",
                 "test_sample_size": 50, "exceptions_found": 8}
            ]
        })
        data = resp.json()
        assert resp.status_code == 200
...
class TestRegulatoryComplianceService:
    ...
    def test_regulation_lifecycle(self):
        reg = self.client.post("/regulations", json={
            "company_id": "comp-1", "regulation_name": "IFRS 15 Revenue",
            "jurisdiction": "ZW", "framework": "IFRS",
            "requirement": "Recognize revenue when performance obligation satisfied",
            "status": "pending_review", "risk_if_non_compliant": "high"
        })
        reg_id = reg.json()["id"]
        
        updated = self.client.post(f"/regulations/{reg_id}/update", params={"company_id": "comp-1", "status": "compliant"})
...
class TestXBRLReportingService:
    ...
    def test_xbrl_generation(self):
        resp = self.client.post("/generate", json={
            "company_id": "comp-1", "fiscal_year": 2026,
            "entity_name": "Vimbai Ltd", "entity_identifier": "ZW0001",
            "taxonomy": "ifrs-full",
            "concepts": [
    ...
    def test_missing_concepts(self):
        resp = self.client.post("/generate", json={
            "company_id": "comp-1", "fiscal_year": 2026,
            "entity_name": "Test Co", "entity_identifier": "TEST001",
...
class TestSubscriptionPlansService:
    ...
    def test_plan_and_subscribe(self):
        plan = self.client.post("/plans", json={
            "tier": "professional", "name": "Pro Plan", "price_monthly": 199,
            "features": ["Multi-company", "Advanced reporting"], "max_users": 50,
            "max_companies": 10, "api_calls_per_month": 10000
        })
    ...
    def test_upgrade(self):
        resp = self.client.post("/upgrade", json={
            "company_id": "comp-1", "current_plan": "basic", "target_plan": "professional",
            "current_period_end": "2026-10-01T00:00:00Z", "prorate": True
        })
        data = resp.json()
        assert resp.status_code == 200
