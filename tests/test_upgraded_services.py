    ...
    def test_execute_rule(self):
        create = self.client.post(
            ...
            json={
                ...
                "company_id": "comp-1",
                "trigger": "manual",
                "steps": [{"step_id": "s1", "step_name": "Step 1", "action": "do_something"}],
            },
        )
...
class TestDashboardService:
    ...
    def test_executive_dashboard(self):
        resp = self.client.post(
            "/load", json={"company_id": "comp-1", "dashboard_type": "executive", "refresh_interval": 30}
        )
        data = resp.json()
        assert resp.status_code == 200
    ...
    def test_financial_dashboard(self):
        resp = self.client.post(
            "/load", json={"company_id": "comp-1", "dashboard_type": "financial", "refresh_interval": 60}
        )
        data = resp.json()
...
class TestEnterpriseSSOService:
    ...
    def test_oidc_auth(self):
        ...
        payload = (
            base64.urlsafe_b64encode(json.dumps({"sub": "user123", "exp": 99999999999}).encode()).rstrip(b"=").decode()
        )
        token = f"{header}.{payload}.signature"

        resp = self.client.post("/auth/sso", json={"organization_id": "org-1", "idp_token": token, "provider": "oidc"})
        data = resp.json()
        assert resp.status_code == 200
    ...
    def test_invalid_token(self):
        resp = self.client.post(
            "/auth/sso", json={"organization_id": "org-1", "idp_token": "short", "provider": "oidc"}
        )
    ...
    def test_session_validation(self):
        ...
        header = base64.urlsafe_b64encode(json.dumps({"alg": "RS256"}).encode()).rstrip(b"=").decode()
        payload = (
            base64.urlsafe_b64encode(json.dumps({"sub": "user456", "exp": 99999999999}).encode()).rstrip(b"=").decode()
        )
...
class TestForeignExchangeService:
    ...
    def test_conversion(self):
        resp = self.client.post("/convert", json={"from_currency": "USD", "to_currency": "EUR", "amount": 100})
        data = resp.json()
        assert resp.status_code == 200
        assert data["rate"] > 0
