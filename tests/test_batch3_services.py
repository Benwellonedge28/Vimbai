...
class TestTaxRiskService:
    ...
    def test_assess(self):
        resp = self.client.post("/assess", json={
            "company_id": "comp-1", "risks": [
                {"description": "TP gap", "risk_type": "transfer_pricing",
                 "potential_exposure": 200000, "probability": 0.4},
                {"description": "VAT gap", "risk_type": "vat_gap",
                 "potential_exposure": 50000, "probability": 0.3}
            ]
        })
        data = resp.json()
...
class TestTaxComplianceService:
    ...
    def test_obligation_lifecycle(self):
        obl = self.client.post("/obligations", json={
            "company_id": "comp-1", "obligation_type": "vat_return",
            "description": "Q1 VAT Return", "due_date": "2026-04-30T00:00:00Z",
            "amount": 15000, "filing_frequency": "quarterly"
        })
...
class TestTaxProvisionService:
    ...
    def test_provision_calculation(self):
        resp = self.client.post("/calculate", json={
            ...
            "pre_tax_income": 500000, "statutory_rate": 0.25,
            ...
            "loss_carryforward": 0, "valuation_allowance": 0
        })
        data = resp.json()
        assert resp.status_code == 200
...
class TestTaxPlanningService:
    ...
    def test_plan(self):
        resp = self.client.post("/plan", json={
            "company_id": "comp-1", "fiscal_year": 2026,
            "current_taxable_income": 1000000, "current_tax": 250000,
            "strategies": [
                {"name": "R&D Credit", "description": "Claim R&D credits",
                 "strategy_type": "credit", "estimated_savings": 50000,
                 "implementation_cost": 5000, "risk_level": "low"},
                {"name": "Accelerated Depreciation", "description": "Use accelerated depreciation",
                 "strategy_type": "timing", "estimated_savings": 30000,
                 "implementation_cost": 2000, "risk_level": "low"}
            ]
        })
        data = resp.json()
        assert resp.status_code == 200
...
class TestRDTaxService:
    ...
    def test_credit_calculation(self):
        resp = self.client.post("/calculate", json={
            ...
            "credit_rate": 0.15, "alternative_rate": 0.20
        })
        data = resp.json()
        assert resp.status_code == 200
...
class TestGroupTaxService:
    ...
    def test_consolidation(self):
        resp = self.client.post("/consolidate", json={
            "group_id": "grp-1", "fiscal_year": 2026,
            "subsidiaries": [
                {"entity_id": "e1", "entity_name": "Sub A", "jurisdiction": "ZW",
                 "pre_tax_income": 300000, "tax_paid": 75000, "tax_rate": 0.25},
                {"entity_id": "e2", "entity_name": "Sub B", "jurisdiction": "ZA",
                 "pre_tax_income": -50000, "tax_paid": 0, "tax_rate": 0.28}
            ],
            "group_tax_rate": 0.25, "intercompany_eliminations": 20000
        })
        data = resp.json()
        assert resp.status_code == 200
...
class TestTransferPricingService:
    ...
    def test_analysis(self):
        resp = self.client.post("/analyze", json={
            "company_id": "comp-1", "transactions": [
                {"company_id": "comp-1", "product_service": "Consulting", "selling_entity": "Parent",
                 "buying_entity": "Sub", "transaction_value": 100000,
                 "arm_length_range_min": 90000, "arm_length_range_max": 120000,
                 "method": "comparable_uncontrolled_price"},
                {"company_id": "comp-1", "product_service": "License", "selling_entity": "Parent",
                 "buying_entity": "Sub", "transaction_value": 50000,
                 "arm_length_range_min": 70000, "arm_length_range_max": 90000,
...
class TestTradeFinanceService:
    ...
    def test_lc_lifecycle(self):
        inst = self.client.post("/instruments", json={
            "company_id": "comp-1", "instrument_type": "letter_of_credit",
            "counterparty": "Overseas Supplier", "amount": 200000,
            "currency": "USD", "issuing_bank": "Stanbic"
        })
        data = inst.json()
...
class TestWorkingCapitalService:
    ...
    def test_analysis(self):
        resp = self.client.post("/analyze", json={
            ...
            "inventory": 150000, "accounts_receivable": 200000, "accounts_payable": 100000,
            "annual_revenue": 2000000, "cogs": 1200000
        })
        data = resp.json()
        assert resp.status_code == 200
    ...
    def test_factoring(self):
        resp = self.client.post("/factoring", json={
            "company_id": "comp-1", "invoice_amount": 100000,
            "advance_rate": 0.85, "factor_fee_rate": 0.03, "discount_rate": 0.02
        })
        data = resp.json()
        assert resp.status_code == 200
...
class TestTargetCostingService:
    ...
    def test_target_cost(self):
        resp = self.client.post("/calculate", json={
            "company_id": "comp-1", "product_name": "WidgetX",
            "target_selling_price": 100, "desired_profit_margin_pct": 20,
            "current_cost": 90,
...
class TestThroughputAccountingService:
    ...
    def test_optimal_mix(self):
        resp = self.client.post("/analyze", json={
            "company_id": "comp-1", "operating_expenses": 5000,
            "products": [
                {"name": "Product A", "selling_price": 100, "material_cost": 40,
                 "time_on_constraint": 2, "demand": 100},
                {"name": "Product B", "selling_price": 80, "material_cost": 20,
                 "time_on_constraint": 4, "demand": 50}
            ],
...
class TestEquivalentUnitsService:
    ...
    def test_weighted_average(self):
        resp = self.client.post("/calculate", json={
            ...
            "units_started": 10000, "units_completed": 9000,
            "ending_wip_units": 1000, "ending_wip_completion_materials": 1.0,
            "ending_wip_completion_conversion": 0.5,
            "materials_cost_beginning": 5000, "conversion_cost_beginning": 3000,
            "materials_cost_added": 95000, "conversion_cost_added": 47000
        })
        data = resp.json()
        assert resp.status_code == 200
...
class TestSegmentReportingService:
    ...
    def test_report(self):
        resp = self.client.post("/report", json={
            "company_id": "comp-1", "fiscal_year": 2026,
            "segments": [
                {"name": "Retail", "segment_type": "business",
                 "revenue": 600000, "expenses": 400000, "profit_or_loss": 200000, "assets": 500000},
                {"name": "Wholesale", "segment_type": "business",
                 "revenue": 300000, "expenses": 250000, "profit_or_loss": 50000, "assets": 200000},
                {"name": "Other", "segment_type": "business",
                 "revenue": 50000, "expenses": 45000, "profit_or_loss": 5000, "assets": 30000}
            ],
            "reconciliation_revenue": 20000, "reconciliation_expenses": 15000
        })
        data = resp.json()
        assert resp.status_code == 200
