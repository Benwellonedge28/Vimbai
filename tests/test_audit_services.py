...
class TestForensicAccounting:
    ...
    def test_engagement_lifecycle(self, forensic_client):
        # Create
        create = forensic_client.post("/engagements", json={
            "company_id": "comp-1", "audit_type": "forensic",
            "title": "Fraud Investigation Q1", "scope": "Financial records",
        ...
        # Add findings
        finding = forensic_client.post(f"/engagements/{eng_id}/findings", json={
            "title": "Misappropriation", "description": "Funds diverted",
            "severity": "critical", "recommendation": "Terminate employee, recover funds"
        })
...
class TestITAudit:
    ...
    def test_audit_with_findings(self, it_audit_client):
        create = it_audit_client.post("/engagements", json={
            "company_id": "comp-2", "audit_type": "it",
            "title": "IT Security Audit", "scope": "Access controls and data security"
        })
        eng_id = create.json()["id"]
        it_audit_client.post(f"/engagements/{eng_id}/findings", json={
            "title": "Weak Passwords", "severity": "high",
            "description": "Many users have weak passwords"
        })
...
class TestOperationalAudit:
    ...
    def test_create_engagement(self, operational_audit_client):
        resp = operational_audit_client.post("/engagements", json={
            "company_id": "comp-3", "audit_type": "operational",
            "title": "Process Audit", "scope": "Manufacturing processes"
        })
...
class TestTaxAudit:
    ...
    def test_full_audit_cycle(self, tax_audit_client):
        create = tax_audit_client.post("/engagements", json={
            "company_id": "comp-4", "audit_type": "tax",
            "title": "Tax Compliance Audit 2026"
        })
        eng_id = create.json()["id"]
        # Add findings
        tax_audit_client.post(f"/engagements/{eng_id}/findings", json={
            "title": "Under-reported income", "severity": "high",
            "description": "Income not fully reported"
        })
        # Complete
...
class TestProcessCosting:
    ...
    def test_calculate_cost(self, process_costing_client):
        resp = process_costing_client.post("/calculate", json={
            "company_id": "comp-1", "product_or_process": "Assembly Line A",
            "period": "2026-01", "quantity": 1000,
            "components": [
    ...
    def test_cost_breakdown(self, process_costing_client):
        calc = process_costing_client.post("/calculate", json={
            "company_id": "comp-bd", "product_or_process": "Product X", "quantity": 100,
            "components": [
...
class TestProductCosting:
    ...
    def test_calculate(self, product_costing_client):
        resp = product_costing_client.post("/calculate", json={
            "company_id": "comp-1", "product_or_process": "Widget Pro",
            "quantity": 500,
            "components": [
...
class TestJobCosting:
    ...
    def test_job_lifecycle(self, job_costing_client):
        # Create job
        create = job_costing_client.post("/jobs", json={
            "company_id": "comp-1", "job_name": "Custom Build",
            "customer": "Client A", "contract_value": 100000
        })
        job_id = create.json()["id"]

        # Add costs
        job_costing_client.post(f"/jobs/{job_id}/costs", json={
            "cost_type": "materials", "amount": 30000, "description": "Building materials"
        })
        job_costing_client.post(f"/jobs/{job_id}/costs", json={
            "cost_type": "labor", "amount": 20000, "description": "Construction labor"
        })
        job_costing_client.post(f"/jobs/{job_id}/costs", json={
            "cost_type": "overhead", "amount": 10000, "description": "Overhead allocation"
        })
