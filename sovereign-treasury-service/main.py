...
try:
    from shared.tracing import setup_tracing; TRACER = setup_tracing(service_name="sovereign-treasury-service", instrument_app=app)
except ImportError:
    TRACER = None
...
async def get_fiscal_position(country: str):
    if country not in _positions: raise HTTPException(status_code=404, detail="No fiscal position found")
