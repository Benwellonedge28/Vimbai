"""
Preference Shares Service CRUD Operations

Neo4j-backed persistence driven generically by the pydantic models:
preference share classes, dividends, and redemption entries map to
...
from preference_shares_service.dependencies import book_id_var
from preference_shares_service.exceptions import NotFoundError, ValidationError
from preference_shares_service.models import PreferenceDividend, PreferenceShareClass, RedemptionEntry
...
# ============================================================================
# Store: Preference Share Classes
# ============================================================================


async def create_share_class(session, user_id, share_class: PreferenceShareClass) -> PreferenceShareClass:
    return await _create(
        session, user_id, "PreferenceShareClassNode", "OWNS_PREF_SHARE_CLASS", PreferenceShareClass, share_class
    )


async def get_share_class(session, user_id, share_class_id: str) -> Optional[PreferenceShareClass]:
    return await _get(
        session, user_id, "PreferenceShareClassNode", "OWNS_PREF_SHARE_CLASS", PreferenceShareClass, share_class_id
    )


async def list_share_classes(session, user_id) -> List[PreferenceShareClass]:
    return await _list(session, user_id, "PreferenceShareClassNode", "OWNS_PREF_SHARE_CLASS", PreferenceShareClass)


async def save_share_class(session, user_id, share_class: PreferenceShareClass) -> None:
    await _save(
        session, user_id, "PreferenceShareClassNode", "OWNS_PREF_SHARE_CLASS", PreferenceShareClass, share_class
    )
...
# ============================================================================
# Store: Preference Dividends
# ============================================================================


async def create_dividend(session, user_id, dividend: PreferenceDividend) -> PreferenceDividend:
    return await _create(session, user_id, "PreferenceDividendNode", "OWNS_PREF_DIVIDEND", PreferenceDividend, dividend)


async def get_dividend(session, user_id, dividend_id: str) -> Optional[PreferenceDividend]:
    return await _get(session, user_id, "PreferenceDividendNode", "OWNS_PREF_DIVIDEND", PreferenceDividend, dividend_id)


async def list_dividends(session, user_id) -> List[PreferenceDividend]:
    return await _list(session, user_id, "PreferenceDividendNode", "OWNS_PREF_DIVIDEND", PreferenceDividend)


async def save_dividend(session, user_id, dividend: PreferenceDividend) -> None:
    await _save(session, user_id, "PreferenceDividendNode", "OWNS_PREF_DIVIDEND", PreferenceDividend, dividend)


# ============================================================================
# Store: Redemption Entries
# ============================================================================


async def create_redemption(session, user_id, redemption: RedemptionEntry) -> RedemptionEntry:
    return await _create(
        session, user_id, "PreferenceRedemptionNode", "OWNS_PREF_REDEMPTION", RedemptionEntry, redemption
    )


async def list_redemptions(session, user_id) -> List[RedemptionEntry]:
    return await _list(session, user_id, "PreferenceRedemptionNode", "OWNS_PREF_REDEMPTION", RedemptionEntry)
