"""
Ordinary Shares Service CRUD Operations

Neo4j-backed persistence driven generically by the pydantic models:
dividends map to labeled nodes with user ownership (X-User-Id) and
book_id stamping. Every read applies the Book filter
`WHERE ($book_id IS NULL OR x.book_id = $book_id)`.

Record-keeping only: this service records dividend declarations and
payment journal-entry references; it never moves money. The /pay
endpoint records that a dividend was paid. Corrections use reversing
entries.
"""
...
from ordinary_shares_service.dependencies import book_id_var
from ordinary_shares_service.exceptions import NotFoundError, ValidationError
from ordinary_shares_service.models import OrdinaryDividend
...
# ============================================================================
# Store: Ordinary Dividends
# ============================================================================


async def create_dividend(session, user_id, dividend: OrdinaryDividend) -> OrdinaryDividend:
    return await _create(session, user_id, "OrdinaryDividendNode", "OWNS_ORDINARY_DIVIDEND", OrdinaryDividend, dividend)


async def get_dividend(session, user_id, dividend_id: str) -> Optional[OrdinaryDividend]:
    return await _get(session, user_id, "OrdinaryDividendNode", "OWNS_ORDINARY_DIVIDEND", OrdinaryDividend, dividend_id)


async def list_dividends(session, user_id) -> List[OrdinaryDividend]:
    return await _list(session, user_id, "OrdinaryDividendNode", "OWNS_ORDINARY_DIVIDEND", OrdinaryDividend)


async def save_dividend(session, user_id, dividend: OrdinaryDividend) -> None:
    await _save(session, user_id, "OrdinaryDividendNode", "OWNS_ORDINARY_DIVIDEND", OrdinaryDividend, dividend)
