...
try:
    from shared.tracing import setup_tracing; TRACER = setup_tracing(service_name="financial-state-machine-service", instrument_app=app)
except ImportError:
    TRACER = None

class DocumentState(str, Enum):
    DRAFT = "draft"; PENDING_APPROVAL = "pending_approval"; APPROVED = "approved"; POSTED = "posted"; CANCELLED = "cancelled"; ARCHIVED = "archived"
...
async def get_document(doc_id: str):
    if doc_id not in _documents: raise HTTPException(status_code=404, detail="Document not found")
...
async def transition(doc_id: str, to_state: DocumentState, user_id: str = "", notes: str = ""):
    ...
    if to_state not in allowed:
        raise HTTPException(status_code=400, detail=f"Invalid transition: {doc.current_state.value} -> {to_state.value}. Allowed: {[s.value for s in allowed]}")
    transition_record = StateTransition(document_id=doc_id, from_state=doc.current_state, to_state=to_state, user_id=user_id, notes=notes)
...
async def get_states():
    return {"states": [s.value for s in DocumentState], "transitions": {k.value: [v.value for v in vs] for k, vs in TRANSITIONS.items()}}
