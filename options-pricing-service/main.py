...
async def price_options(request: OptionsPricingRequest):
    ...
    for opt in request.options:
        ...
        theta = (
            ...
            - r * K * math.exp(-r * T) * norm.cdf(d1 if opt.option_type == "call" else -d1)
        ) / 365
        ...
        option_valuations.append(
            {
                ...
                "moneyness": (
                    "ITM"
                    if (opt.option_type == "call" and S > K) or (opt.option_type == "put" and S < K)
    ...
    portfolio_summary = {
        ...
        "avg_premium_pct": (
            round(total_premium / (sum(o.spot_price * o.notional for o in request.options)) * 100, 4)
            if request.options
            else 0
        ),
    }
