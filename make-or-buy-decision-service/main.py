...
class MakeCost(BaseModel):
    direct_materials: float; direct_labour: float; variable_overhead: float
    fixed_overhead: float; setup_cost: float = 0; tooling_cost: float = 0

class BuyCost(BaseModel):
    unit_purchase_price: float; ordering_cost: float = 0
    carrying_cost_per_unit: float = 0; quality_inspection_cost: float = 0

class MakeOrBuyRequest(BaseModel):
    company_id: str; product_name: str; annual_volume: int
    make_costs: MakeCost; buy_costs: BuyCost
    opportunity_cost: float = 0; quality_considerations: str = ""

class MakeOrBuyResult(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; product_name: str; annual_volume: int
    make_total_cost: float; buy_total_cost: float; difference: float
    recommendation: str; make_cost_per_unit: float; buy_cost_per_unit: float
...
async def analyze_make_or_buy(req: MakeOrBuyRequest):
    ...
    make_total = (req.make_costs.direct_materials + req.make_costs.direct_labour + req.make_costs.variable_overhead) * vol + req.make_costs.fixed_overhead + req.make_costs.setup_cost + req.make_costs.tooling_cost + req.opportunity_cost
    
    buy_total = (req.buy_costs.unit_purchase_price + req.buy_costs.quality_inspection_cost) * vol + req.buy_costs.ordering_cost + (req.buy_costs.carrying_cost_per_unit * vol)
    ...
    return MakeOrBuyResult(
        ...
        annual_volume=vol,
        make_total_cost=round(make_total, 2), buy_total_cost=round(buy_total, 2),
        difference=round(difference, 2), recommendation=recommendation,
        make_cost_per_unit=round(make_per_unit, 2), buy_cost_per_unit=round(buy_per_unit, 2),
