...
async def calculate_npv(
    ...
    return {
        ...
        "decision": "Accept - Positive NPV" if npv > 0 else "Reject - Negative NPV" if npv < 0 else "Indifferent - Zero NPV"
    }
...
async def compare_npv_projects(
    projects: List[dict]  # [{"name": "A", "initial_investment": x, "cash_flows": [y1, y2, ...], "discount_rate": r, "residual_value": v}]
):
