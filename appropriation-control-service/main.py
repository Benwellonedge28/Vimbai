...
try:
    from shared.tracing import setup_tracing; TRACER = setup_tracing(service_name="appropriation-control-service", instrument_app=app)
except ImportError:
    TRACER = None
...
async def get_appropriations(company_id: str, department: Optional[str] = None):
    ...
    if department: apprs = [a for a in apprs if a.department == department]
    ...
    for apprs in _appropriations.values():
        for a in apprs:
            if a.id == tx.appropriation_id:
                if tx.type == "commit": a.committed_amount += tx.amount
                elif tx.type == "spend": a.spent_amount += tx.amount; a.committed_amount -= tx.amount
                elif tx.type == "uncommit": a.committed_amount -= tx.amount
                elif tx.type == "refund": a.spent_amount -= tx.amount
                a.available_amount = a.approved_amount - a.committed_amount - a.spent_amount
                if a.available_amount <= 0: a.status = "exhausted"
    ...
    for apprs in _appropriations.values():
        for a in apprs:
            if a.id == appropriation_id:
                ...
                return {"appropriation_id": appropriation_id, "available": a.available_amount, "requested": amount, "allowed": can_spend}
    raise HTTPException(status_code=404, detail="Appropriation not found")
