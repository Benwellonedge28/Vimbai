...
async def analyze_accounts_receivable(request: ARAnalysisRequest):
    ...
    aging_schedule = [
        {
            "bucket": "Current",
            "amount": round(current_ar, 2),
            "percentage": round(current_ar / total_ar * 100, 2) if total_ar else 0,
        },
        {
            "bucket": "1-30 Days Overdue",
            "amount": round(overdue_30, 2),
            "percentage": round(overdue_30 / total_ar * 100, 2) if total_ar else 0,
        },
        {
            "bucket": "31-60 Days Overdue",
            "amount": round(overdue_60, 2),
            "percentage": round(overdue_60 / total_ar * 100, 2) if total_ar else 0,
        },
        {
            "bucket": "61-90 Days Overdue",
            "amount": round(overdue_90, 2),
            "percentage": round(overdue_90 / total_ar * 100, 2) if total_ar else 0,
        },
    ]
    ...
    dso_analysis = {
        ...
        "collection_efficiency": (
            round((1 - avg_collection_period / request.target_dso) * 100, 2) if request.target_dso else 0
        ),
    }
    ...
    bad_debt_provision = {
        ...
        "provision_percentage": (
            round((overdue_30 * 5 + overdue_60 * 25 + overdue_90 * 75) / total_ar * 100, 2) if total_ar else 0
        ),
    }
