...
try:
    from shared.tracing import setup_tracing; TRACER = setup_tracing(service_name="equity-changes-service", instrument_app=app)
except ImportError:
    TRACER = None

class EquityTransactionType(str, Enum):
    ISSUANCE = "issuance"; BUYBACK = "buyback"; DIVIDEND = "dividend"; SPLIT = "split"; TRANSFER = "transfer"; RETAINED = "retained_earnings"
...
async def get_transactions(company_id: str, tx_type: Optional[str] = None):
    ...
    if tx_type: txs = [t for t in txs if t.transaction_type.value == tx_type]
...
async def generate_statement(stmt: EquityStatement):
    stmt.share_issuances = sum(t.amount for t in stmt.transactions if t.transaction_type == EquityTransactionType.ISSUANCE)
    stmt.share_buybacks = sum(t.amount for t in stmt.transactions if t.transaction_type == EquityTransactionType.BUYBACK)
    stmt.dividends_paid = sum(t.amount for t in stmt.transactions if t.transaction_type == EquityTransactionType.DIVIDEND)
    stmt.retained_earnings_change = sum(t.amount for t in stmt.transactions if t.transaction_type == EquityTransactionType.RETAINED)
    stmt.other_changes = sum(t.amount for t in stmt.transactions if t.transaction_type in (EquityTransactionType.SPLIT, EquityTransactionType.TRANSFER))
    stmt.ending_equity = stmt.beginning_equity + stmt.share_issuances - stmt.share_buybacks - stmt.dividends_paid + stmt.retained_earnings_change + stmt.other_changes
