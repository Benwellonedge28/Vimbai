"""
FinAcc Flexible Budget Service
...
app = FastAPI(title="FinAcc Flexible Budget Service", version=SERVICE_VERSION, docs_url="/docs")
...
@app.post("/calculate")
async def calculate_flexible_budget(
    budgeted_cost: float,
    budgeted_activity: float,
...
async def flexible_budget_multiple_levels(
    budgeted_fixed_cost: float,
    budgeted_variable_rate: float,
    budgeted_activity: float,
    actual_activity: float
):
...
async def flexible_budget_variance_analysis(
    static_budget: float,
    flexed_budget: float,
    actual_result: float
):
...
async def calculate_flexed_cost_per_unit(
    flexed_budget: float,
    actual_activity: float
):
