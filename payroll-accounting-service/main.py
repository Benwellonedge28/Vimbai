...
class EmployeePayroll(BaseModel):
    employee_id: str; employee_name: str; gross_salary: float
    paye_rate: float = 0.25; nassa_rate: float = 0.03; pension_rate: float = 0.05
    medical_aid: float = 0; other_deductions: float = 0

class PayrollRequest(BaseModel):
    company_id: str; period: str; employees: List[EmployeePayroll]

class PayrollResult(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str; period: str
    total_gross: float; total_net: float; total_paye: float; total_nassa: float
    total_pension: float; total_medical: float; total_other_deductions: float
    employer_pension: float; employer_nassa: float; total_cost_to_company: float
    employee_count: int; journal_entries: List[Dict] = []
...
async def process_payroll(req: PayrollRequest):
    total_gross = 0; total_net = 0; total_paye = 0; total_nassa = 0
    total_pension = 0; total_medical = 0; total_other = 0
    ...
    return PayrollResult(
        ...
        total_paye=round(total_paye, 2), total_nassa=round(total_nassa, 2),
        total_pension=round(total_pension, 2), total_medical=round(total_medical, 2),
        total_other_deductions=round(total_other, 2),
        employer_pension=round(employer_pension, 2), employer_nassa=round(employer_nassa, 2),
        total_cost_to_company=round(total_cost, 2),
        employee_count=len(req.employees), journal_entries=journal_entries
    )
