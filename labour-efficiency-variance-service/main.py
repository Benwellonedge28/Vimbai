...
async def calculate_variance(req: LabourVarianceRequest):
    ...
    eff_var = (
        (req.standard_hours - req.actual_hours) * req.standard_rate
        if req.standard_hours
        else (expected_hours - req.actual_hours) * req.standard_rate
    )
    ...
    if rate_var < 0:
        analysis["rate"] = (
            f"Unfavourable: paying {req.actual_rate} vs standard {req.standard_rate} - negotiate or review pay scales"
        )
    else:
        analysis["rate"] = f"Favourable: paying below standard rate"
    if eff_var < 0:
        analysis["efficiency"] = (
            f"Unfavourable: {req.actual_hours}h actual vs {expected_hours}h expected - training or process improvement needed"
        )
    else:
    ...
    return LabourVarianceResult(
        ...
        idle_time_variance=round(idle_var, 2),
