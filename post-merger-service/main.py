...
async def track_integration(request: PostMergerRequest):
    ...
    synergy_performance = {
        ...
        "daily_rate": round(request.synergies_achieved / request.days_since_close, 2) if request.days_since_close else 0
    }
    
    cost_variance = request.integration_costs_actual - request.integration_costs_budget
    cost_rate = request.integration_costs_actual / request.integration_costs_budget if request.integration_costs_budget else 0
    
    cost_performance = {
        "budget": request.integration_costs_budget,
        "actual": request.integration_costs_actual,
        "variance": round(cost_variance, 2),
        "variance_percentage": round((cost_variance / request.integration_costs_budget * 100) if request.integration_costs_budget else 0, 2),
    ...
    avg_completion = sum(m.completion_percentage for m in request.milestones) / len(request.milestones) if request.milestones else 0
