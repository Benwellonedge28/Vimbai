...
class DashboardConfig(BaseModel):
    company_id: str
    dashboard_type: str  # executive, financial, operations, compliance
...
def _executive_widgets():
    return [
        Widget(
            type="kpi_card", title="Total Revenue", data={"value": 0, "format": "currency", "trend": "up"}, position=1
        ),
        Widget(
            type="kpi_card", title="Net Income", data={"value": 0, "format": "currency", "trend": "down"}, position=2
        ),
...
async def load_dashboard(req: DashboardConfig):
    ...
    return DashboardResponse(
        ...
        dashboard_type=req.dashboard_type,
        widgets=widgets,
