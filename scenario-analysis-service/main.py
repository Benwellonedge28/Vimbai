...
class ScenarioAssumption(BaseModel):
    revenue_growth: float = 0.1; cost_growth: float = 0.05
    interest_rate: float = 0.05; tax_rate: float = 0.25
    capex: float = 0; description: str = ""

class ScenarioRequest(BaseModel):
    company_id: str; base_revenue: float; base_cost: float
    base_interest: float = 0; base_depreciation: float = 0
    best_case: ScenarioAssumption; base_case: ScenarioAssumption; worst_case: ScenarioAssumption

class ScenarioResult(BaseModel):
    name: str; description: str
    projected_revenue: float; projected_cost: float
    ebit: float; pretax_income: float; net_income: float; net_margin: float

class AnalysisResponse(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    company_id: str
    best_case: ScenarioResult; base_case: ScenarioResult; worst_case: ScenarioResult
    sensitivity_revenue: float; sensitivity_cost: float
    recommendation: str
...
def _calc_scenario(name: str, assumption: ScenarioAssumption, base_rev: float, base_cost: float, base_int: float, base_dep: float) -> ScenarioResult:
    ...
    return ScenarioResult(
        name=name, description=assumption.description,
        projected_revenue=round(rev, 2), projected_cost=round(cost, 2),
        ebit=round(ebit, 2), pretax_income=round(pretax, 2),
        net_income=round(net, 2), net_margin=round(margin, 2)
    )

@app.post("/analyze", response_model=AnalysisResponse)
async def analyze_scenarios(req: ScenarioRequest):
    best = _calc_scenario("Best Case", req.best_case, req.base_revenue, req.base_cost, req.base_interest, req.base_depreciation)
    base = _calc_scenario("Base Case", req.base_case, req.base_revenue, req.base_cost, req.base_interest, req.base_depreciation)
    worst = _calc_scenario("Worst Case", req.worst_case, req.base_revenue, req.base_cost, req.base_interest, req.base_depreciation)
    ...
    return AnalysisResponse(
        ...
        best_case=best, base_case=base, worst_case=worst,
...
async def create_scenario(req: ScenarioCreate):
    sid = str(uuid.uuid4())
    scenario = {
        "id": sid, "company_id": req.company_id, "name": req.name,
...
async def compare_scenarios(company_id: str):
    ...
    return {
        "best_case": best["name"], "worst_case": worst["name"],
        "best_net": best["net_projection"], "worst_net": worst["net_projection"],
