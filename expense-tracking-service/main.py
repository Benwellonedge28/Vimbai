...
try:
    ...
    TRACER = setup_tracing(service_name="expense-tracking-service", instrument_app=app)
except ImportError:
    TRACER = None
...
async def get_expenses(
    company_id: str, category: Optional[str] = None, status_filter: Optional[str] = None, limit: int = 100
):
    expenses = _expenses.get(company_id, [])
    if category:
        expenses = [e for e in expenses if e.category.value == category]
    if status_filter:
        expenses = [e for e in expenses if e.status.value == status_filter]
